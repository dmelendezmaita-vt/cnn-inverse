...
BATCHES: Tuple[BatchDefinition, ...] = (
    ...
    BatchDefinition(
        batch_id="smk09_interactive_execution_policy",
        sequence=49,
        title="Interactive execution-policy smoke",
        suite="canonical_complete_smoke",
        scientific_role="engineering_support",
        summary="Runs reduced `interactive_dnn` launchers across 1-node, 2-node, and 4-node layouts, so the smoke suite covers the same distributed launcher class that the full execution-policy batch depends on.",
        requires_hh_tar=True,
        requires_slurm=True,
        cluster_label="Falcon",
        min_nodes=4,
        ...
        steps=(
            BatchStep(
                ...
                description="Single-node interactive HH smoke train",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
                ...
                step_nnodes=1,
                nproc_per_node=1,
                env_overrides=(("DATA_ACCESS_MODE", "nvme_full_extract"),),
                parallel_group="sci02_train_small",
            ),
            BatchStep(
                step_id="one_node_eval",
                kind="interactive_dnn",
                description="Single-node interactive HH smoke eval",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
                ...
                nproc_per_node=1,
                env_overrides=(("DATA_ACCESS_MODE", "nvme_full_extract"),),
                parallel_group="sci02_eval_small",
            ),
            BatchStep(
                step_id="two_node_train",
                kind="interactive_dnn",
                description="Two-node interactive HH smoke train",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
                split_eval_after_train=True,
            ...
            BatchStep(
                ...
                kind="interactive_dnn",
                description="Two-node interactive HH smoke eval",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
            ...
            BatchStep(
                ...
                kind="interactive_dnn",
                description="Four-node interactive HH smoke train",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
                split_eval_after_train=True,
            ...
            BatchStep(
                ...
                kind="interactive_dnn",
                description="Four-node interactive HH smoke eval",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke.yaml",
                pass_curr=True,
                save_predictions="none",
    ...
    BatchDefinition(
        batch_id="smk10_interactive_execution_path",
        sequence=51,
        title="Interactive execution-path smoke",
        suite="canonical_complete_smoke",
        scientific_role="engineering_support",
        summary="Runs reduced cache-enabled `interactive_dnn` launchers with reusable node-local NVMe extraction, so the smoke suite covers the agreed cache-path launcher family that the full execution-path batch depends on.",
        requires_hh_tar=True,
        requires_slurm=True,
        cluster_label="Falcon",
        min_nodes=4,
        preferred_gpu="A30",
        depends_on=("smk09_interactive_execution_policy",),
        steps=(
            BatchStep(
                step_id="cache_nvme_train",
                kind="interactive_dnn",
                description="Cache-path smoke train with reusable node-local NVMe extraction",
            ...
            BatchStep(
                step_id="cache_nvme_eval",
                kind="interactive_dnn",
                description="Cache-path smoke eval with reusable node-local NVMe extraction",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_smoke_cache.yaml",
                pass_curr=True,
                save_predictions="none",
                load_from_step="cache_nvme_train",
                step_nnodes=1,
                nproc_per_node=1,
                env_overrides=(("DATA_ACCESS_MODE", "nvme_full_extract"),),
            ),
    ...
    BatchDefinition(
        ...
        steps=(
            BatchStep(
                ...
                parallel_group="sci04_posterior",
            ),
            BatchStep(
    ...
    BatchDefinition(
        ...
        steps=(
            BatchStep(
                ...
                parallel_group="sci05_robustness",
            ),
            BatchStep(
    ...
    BatchDefinition(
        ...
        steps=(
            BatchStep(
                ...
                parallel_group="sci06_extensions",
            ),
            BatchStep(
    ...
    BatchDefinition(
        ...
        suite="scientific_smoke",
        scientific_role="engineering_support",
        summary="Falcon-sized HH cache-path reproduction with reusable node-local NVMe extraction, so the public execution-path optimizations can be exercised through the agreed distributed launcher and cache settings.",
        requires_hh_tar=True,
        requires_slurm=True,
        cluster_label="Falcon",
        min_nodes=4,
        ...
        steps=(
            BatchStep(
                step_id="cache_nvme_train",
                kind="interactive_dnn",
                description="Cache-enabled HH train with reusable node-local NVMe extraction",
            ...
            BatchStep(
                step_id="cache_nvme_eval",
                kind="interactive_dnn",
                description="Cache-enabled HH eval with reusable node-local NVMe extraction",
                params_file="src/pytorch/configs/hh/params_dnn_tar_hh_cache_campaign.yaml",
                pass_curr=True,
                save_predictions="none",
                load_from_step="cache_nvme_train",
    ...
    BatchDefinition(
        batch_id="can00_falcon_preflight",
        sequence=175,
        title="Canonical complete: Falcon allocation preflight",
        suite="canonical_complete",
        scientific_role="engineering_support",
        summary="Verifies the documented Falcon contract, both Python environments, the HH tarball, and lightweight 1-node, 2-node, and 4-node Slurm step launches before the full canonical suite begins.",
        requires_hh_tar=True,
        requires_slurm=True,
        cluster_label="Falcon",
        min_nodes=4,
        ...
        steps=(
            BatchStep(
                step_id="preflight",
                kind="script",
                description="Falcon preflight checks for the complete canonical suite",
                script_path="scripts/preflight_falcon_repro.py",
                pass_data_dir=True,
                required_globs=("preflight_summary.json",),
                wrap_srun=False,
...
SUITES: Dict[str, Tuple[str, ...]] = {
    ...
    "canonical_complete_smoke": (
        ...
        "smk03_hh_classical",
        "smk04_hh_sbi",
        "smk09_interactive_execution_policy",
        "smk10_interactive_execution_path",
