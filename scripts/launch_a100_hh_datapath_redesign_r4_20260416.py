...
REGISTRY_CSV = TABLES / f"a100_hh_datapath_redesign_r4_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a100_hh_datapath_redesign_r4_run_log_{DATE_TAG}.json"
...
def use_slurm_direct_sidecar(row: Dict[str, str]) -> bool:
    return row.get("strategy_id", "").startswith("R3_postlocalsgd_")
    ...
    env.update(
        {
            ...
            "FOLLOWUP_TMP_BASE": "/projects/neuro-collab/other/a100if_tmp",
        }
    )
    if use_slurm_direct_sidecar(row):
        env.update(
            {
                "SPLIT_EVAL_AFTER_TRAIN": "1",
                "LAUNCH_BACKEND": os.environ.get("R4_A100_POSTLOCALSGD_LAUNCH_BACKEND", "slurm_direct"),
            }
        )
    return env
...
def build_srun_cmd(alloc_job_id, node_slice, row):
    ...
    if use_slurm_direct_sidecar(row):
        ...
        cpus_per_gpu = max(1, int(row["cpus_per_node"]) // max(1, gpus_per_node))
        return [
            "srun",
            ...
            f"-N{n_nodes}",
            f"-w{nodelist}",
            f"--ntasks={n_nodes * gpus_per_node}",
            f"--ntasks-per-node={gpus_per_node}",
            f"--cpus-per-task={cpus_per_gpu}",
            f"--gres=gpu:a100:{row['gpus_per_node']}",
            "--kill-on-bad-exit=1",
            "bash",
            str(STEP_SCRIPT),
        ]
