...
DATE_TAG = "20260418_v100_hh_ddp_tuning_r5c_track3_b50_backfill"
...
MATRIX_CSV = TABLES / f"v100_hh_ddp_tuning_r5c_track3_b50_backfill_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_ddp_tuning_r5c_track3_b50_backfill_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants_by_track = {
        "track3_hh_reduced": [
            ("ddpopt_gabv1_b50", "Track3 bucket backfill: gradient_as_bucket_view on, bucket_cap_mb=50.", None, "True", "50"),
        ],
    }
    ...
    for track_key, meta in track_meta.items():
        for seed in [972, 973]:
            for variant_name, variant_desc, ddp_static_graph, ddp_gabv, ddp_bucket in variants_by_track[track_key]:
                ...
                updates = {
                    "description": f'"{track_key} {variant_name} phase R5c track3 b50 backfill {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        ...
                        "phase": "phaseR5c_ddp_tuning_track3_b50_backfill",
                        ...
                        "notes": "Track3 b50 backfill on the confirmation seeds to compare directly against b100.",
                    }
                )
    ...
    write_text(
        ...
        "\n".join(
            [
                f"# V100 HH DDP Tuning R5c Track3 B50 Backfill ({DATE_LABEL})",
                "",
                f"- matrix csv: `{MATRIX_CSV}`",
                f"- rows: `{len(rows)}`",
                "- scope: 1 track x 1 variant x 2 seeds",
                "",
                "This backfill package compares `b50` to the already-run baseline and `b100` rows:",
                "- `track3_hh_reduced`: `ddpopt_gabv1_b50`",
                "- seeds: `972`, `973`",
            ]
        )
        + "\n"
