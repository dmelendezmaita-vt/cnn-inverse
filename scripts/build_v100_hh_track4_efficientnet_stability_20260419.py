...
DATE_TAG = "20260419_v100_hh_track4_efficientnet_stability"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_efficientnet_stability_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_efficientnet_stability_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 EfficientNet stability baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "effnet_beta005_invstd": {
            "description": "Track4 EfficientNet stability candidate: beta=0.05 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta01_invstd": {
            "description": "Track4 EfficientNet stability candidate: beta=0.1 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
    ...
    rows = []
    row_num = 0
    for seed in [1040, 1041, 1042, 1043, 1044, 1045]:
        group = f"track4_effnet_stability_seed_{seed}"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} efficientnet stability {DATE_LABEL}"',
                "type": variant["type"],
            ...
            rows.append(
                {
                    "row_id": f"v100t4effs_{row_num:04d}",
                    "phase": "phaseAG_track4_efficientnet_stability",
                    "phase_label": "Track4 EfficientNet stability",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_efficientnet_stability",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Wider fresh-seed stability pass between the ConvNet baseline and the two strongest EfficientNet objective variants.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 EfficientNet Stability ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- stress-test whether EfficientNet truly beats the ConvNet baseline over a wider fresh-seed block",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `effnet_beta005_invstd`",
