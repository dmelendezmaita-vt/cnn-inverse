...
DATE_TAG = "20260418_v100_hh_accuracy_batch_epoch_sidecar"
...
MATRIX_CSV = TABLES / f"v100_hh_accuracy_batch_epoch_sidecar_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_accuracy_batch_epoch_sidecar_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    row_num = 0
    variants = {
        # Base quality candidate to compare against the batch/epoch changes.
        "be_base_g128_e400": {
            "description": "Track3 batch/epoch sidecar baseline: gbs128 epochs400.",
        ...
        "be_g64_e400": {
            "description": "Track3 batch/epoch sidecar: smaller global batch, same epochs.",
            "learning_rate": "3.0e-4",
            "epochs": "400",
            "init_learning_rate": "1.5e-5",
            "linear_epochs": "40",
        ...
        "be_g128_e600": {
            "description": "Track3 batch/epoch sidecar: same batch, longer schedule.",
            "learning_rate": "3.0e-4",
            "epochs": "600",
            "init_learning_rate": "1.5e-5",
            "linear_epochs": "60",
        ...
        "be_g64_e600": {
            "description": "Track3 batch/epoch sidecar: smaller batch and longer schedule.",
            "learning_rate": "3.0e-4",
            "epochs": "600",
            "init_learning_rate": "1.5e-5",
            "linear_epochs": "60",
    ...
    for seed in [971, 972, 973]:
        group = f"track3_seed{seed}_batch_epoch"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} batch/epoch sidecar {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100be_{row_num:04d}",
                    "phase": "phaseC_track3_batch_epoch_sidecar",
                    "phase_label": "Track3 batch/epoch sidecar",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "accuracy_batch_epoch_sidecar",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Batch/epoch sidecar for the winner recipe family; isolates training-budget effects after the first accuracy wave.",
                }
            )
    ...
    notes = [
        f"# V100 HH Accuracy Batch/Epoch Sidecar ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- measure whether accuracy gains come from smaller batch noise, more epochs, or both",
        "- keep the datapath stack and loss family fixed",
        "",
        "## Current design",
        "- track3_hh_reduced only",
        "- global_train_batch_size: {64, 128}",
        "- epochs: {400, 600}",
        "- seeds: {971, 972, 973}",
        "",
        "## Note",
        "- this is a sidecar for the winning quality recipe family; do not mix it into the first accuracy-wave decision package",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
