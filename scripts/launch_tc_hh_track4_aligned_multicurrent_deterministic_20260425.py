...
REGISTRY_CSV = TABLES / f"tc_hh_track4_aligned_multicurrent_deterministic_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"tc_hh_track4_aligned_multicurrent_deterministic_run_log_{DATE_TAG}.json"
STEP_SCRIPT = REPO / "scripts" / "run_tc_hh_track4_aligned_multicurrent_deterministic_20260425.sh"
    ...
    env.update(
        {
            ...
            "SVD_COMPONENTS": row["svd_components"],
...
def main() -> None:
    ...
    for row in rows:
        ...
        ended_at = now_iso()
        elapsed = ""
        existing = [r for r in registry if r["row_id"] != row["row_id"]]
        existing.append(
            {
                "row_id": row["row_id"],
