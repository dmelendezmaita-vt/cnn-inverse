...
DATE_TAG = "20260418_v100_hh_ddp_tuning_r5c_track3_bucket_followup"
...
MATRIX_CSV = TABLES / f"v100_hh_ddp_tuning_r5c_track3_bucket_followup_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_ddp_tuning_r5c_track3_bucket_followup_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants_by_track = {
        "track3_hh_reduced": [
            ("ddpopt_gabv1_b25", "Track3 bucket follow-up: gradient_as_bucket_view on, bucket_cap_mb=25.", None, "True", "25"),
            ("ddpopt_gabv1_b50", "Track3 bucket follow-up: gradient_as_bucket_view on, bucket_cap_mb=50.", None, "True", "50"),
        ],
    }
    ...
    for track_key, meta in track_meta.items():
        for seed in [974, 975, 976]:
            for variant_name, variant_desc, ddp_static_graph, ddp_gabv, ddp_bucket in variants_by_track[track_key]:
                ...
                updates = {
                    "description": f'"{track_key} {variant_name} phase R5c track3 bucket follow-up {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        ...
                        "phase": "phaseR5c_ddp_tuning_track3_bucket_followup",
                        ...
                        "notes": "Track3 bucket follow-up comparing the missing b25/b50 rows on already-measured seeds.",
                    }
                )
    ...
    write_text(
        ...
        "\n".join(
            [
                f"# V100 HH DDP Tuning R5c Track3 Bucket Follow-Up ({DATE_LABEL})",
                "",
                f"- matrix csv: `{MATRIX_CSV}`",
                f"- rows: `{len(rows)}`",
                "- scope: 1 track x 2 variants x 3 seeds",
                "",
                "This follow-on package fills the missing track3 bucket variants:",
                "- `track3_hh_reduced`: `ddpopt_gabv1_b25` and `ddpopt_gabv1_b50`",
                "- seeds: `974`, `975`, `976`",
            ]
        )
        + "\n"
