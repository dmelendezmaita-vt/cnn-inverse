...
def started_at_from_stdout(stdout_log: Path) -> str | None:
...
def clean_notes(existing_notes: str | None) -> str:
    note = (existing_notes or "").replace("dry-run only", "")
    parts = [part.strip() for part in note.split(";") if part.strip()]
    if "reconciled_from_run_dir" not in parts:
        parts.append("reconciled_from_run_dir")
    return "; ".join(parts)
...
def main():
    ...
    for row_id, row in matrix_rows.items():
        ...
        stdout_log = LOGS / f"{row_id}.out"
        started_at = started_at_from_stdout(stdout_log)
        ...
        registry_rows[row_id] = {
            ...
            "notes": clean_notes(existing.get("notes", "") if existing else ""),
        }
        reconciled += 1
