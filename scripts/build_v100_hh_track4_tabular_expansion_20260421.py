...
DATE_TAG = "20260421_v100_hh_track4_tabular_expansion"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_tabular_expansion_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_tabular_expansion_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    block_a = [
        ("extra_trees_500", "Current clean-data ExtraTrees anchor (best MSE-oriented tree baseline)."),
        ("knn_k11", "Current robustness-oriented kNN anchor."),
        ("random_forest_500", "Bagged-tree baseline to test whether extra randomization is still helping relative to plain forests."),
        ("hist_gbrt_300_depth6", "Gradient-boosted histogram trees as the strongest missing in-stack tabular family available in sklearn."),
    ]
    block_b = [
        ("svd128_mlp_512_256", "Reduced-feature MLPRegressor baseline to probe a purely tabular neural regressor."),
        ("svd128_svr_rbf", "Kernel baseline on reduced embeddings."),
    ]

    rows = []
    row_num = 0
    for seed in [1107, 1108, 1109, 1110]:
        for block_name, baselines in [("a", block_a), ("b", block_b)]:
            group = f"track4_v100_tabular_expansion_seed_{seed}_block_{block_name}"
            for baseline_name, baseline_desc in baselines:
                ...
                rows.append(
                    {
                        "row_id": f"v100t4tab_{row_num:04d}",
                        "phase": "phaseCP1_track4_v100_tabular_expansion",
                        "phase_label": "Track4 V100 tabular expansion",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track4_v100_tabular_expansion",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "cpus_per_node": "24",
                        ...
                        "notes": "Checkpoint 1 current-stack expansion: stronger sklearn tabular families compared against the current ExtraTrees and kNN anchors on fresh seeds.",
                    }
                )
    ...
    notes = [
        f"# V100 HH Track4 Tabular Expansion ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- start Checkpoint 1 by expanding the current-stack tabular frontier with stronger sklearn families while keeping the current ExtraTrees and kNN anchors in the same package",
        "",
        "## Baselines",
        *[f"- `{name}`" for name, _ in (block_a + block_b)],
        "",
        "## Seeds",
        "- `1107`",
        "- `1108`",
