...
NOTES_MD = NOTES / f"v100_hh_track4_convresnet_retune_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in convresnet builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 convresnet retune baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "convresnet_shallow": {
            "description": "Track4 convresnet retune: shallow ConvResNet.",
            "type": '"ConvResNet"',
            "conv_layer_sizes": "[8, 8, 8, 8]",
            "conv_layer_kernel": "3",
            "conv_layer_stride": "2",
            "conv_layer_padding": "1",
            "conv_layer_padding_mode": '"replicate"',
            "residual_blocks_sizes": "[[32, 32, 128, 32]]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
        },
        "convresnet_deeper": {
            "description": "Track4 convresnet retune: deeper ConvResNet.",
            "type": '"ConvResNet"',
            "conv_layer_sizes": "[8, 8, 8, 8, 8]",
            "conv_layer_kernel": "3",
            "conv_layer_stride": "2",
            "conv_layer_padding": "1",
            "conv_layer_padding_mode": '"replicate"',
            "residual_blocks_sizes": "[[32, 32, 128, 32], [32, 32, 128, 32]]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
        },
    }
    ...
    rows = []
    row_num = 0
    for seed in [1021, 1022, 1023, 1024]:
        group = f"track4_seed{seed}_convresnet"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} convresnet retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4cr_{row_num:04d}",
                    "phase": "phaseAA_track4_convresnet_retune",
                    "phase_label": "Track4 ConvResNet retune",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_convresnet_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Alternate backbone family after stabilizing the track4 avg-pool winner.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 ConvResNet Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether residual convolutional backbones improve the stabilized track4 winner",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `convresnet_shallow`",
