...
DATE_TAG = "20260420_a30_hh_track4_gpu_profile"
...
MATRIX_CSV = TABLES / f"a30_hh_track4_gpu_profile_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a30_hh_track4_gpu_profile_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 A30 GPU profile baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "effnet_beta005_invstd": {
            "description": "Track4 A30 GPU profile candidate: EfficientNet beta=0.05 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta01_invstd": {
            "description": "Track4 A30 GPU profile candidate: EfficientNet beta=0.1 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta005_invvar": {
            "description": "Track4 A30 GPU profile candidate: EfficientNet beta=0.05 inverse_var.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
    ...
    rows = []
    row_num = 0
    variants = {k: v for k, v in variants.items() if k in {"avgpool_baseline", "effnet_beta005_invstd", "effnet_beta005_invvar"}}

    seeds = [1062, 1063]
    for seed in seeds:
        group = f"track4_a30_gpu_profile_seed_{seed}"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} a30 gpu profile {DATE_LABEL}"',
                "type": variant["type"],
            ...
            rows.append(
                {
                    "row_id": f"a30t4gpu_{row_num:04d}",
                    "phase": "phaseAL_track4_a30_gpu_profile",
                    "phase_label": "Track4 A30 GPU profile",
                    "launch_mode": "dedicated_1x1n",
                    "launch_group": "",
                    "family": "track4_a30_gpu_profile",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "A30-side GPU utilization profile for the strongest Track4 single-model candidates.",
                }
            )
    ...
    notes = [
        f"# A30 HH Track4 GPU Profile ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- run a small A30 profiling package with GPU monitoring enabled for the strongest Track4 single-model candidates",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `effnet_beta005_invstd`",
