...
DATE_TAG = "20260420_a30_hh_track4_replication"
...
MATRIX_CSV = TABLES / f"a30_hh_track4_replication_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a30_hh_track4_replication_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 A30 replication baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "effnet_beta005_invstd": {
            "description": "Track4 A30 replication candidate: EfficientNet beta=0.05 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta01_invstd": {
            "description": "Track4 A30 replication candidate: EfficientNet beta=0.1 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta005_invvar": {
            "description": "Track4 A30 replication candidate: EfficientNet beta=0.05 inverse_var.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
    ...
    rows = []
    row_num = 0
    for seed in [1058, 1059, 1060, 1061]:
        group = f"track4_a30_replication_seed_{seed}"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} a30 replication {DATE_LABEL}"',
                "type": variant["type"],
            ...
            rows.append(
                {
                    "row_id": f"a30t4rep_{row_num:04d}",
                    "phase": "phaseAK_track4_a30_replication",
                    "phase_label": "Track4 A30 replication",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_a30_replication",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "A30-side replication of the strongest Track4 single-model candidates.",
                }
            )
    ...
    notes = [
        f"# A30 HH Track4 Replication ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- replicate the strongest Track4 single-model candidates on the live A30 allocation",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `effnet_beta005_invstd`",
