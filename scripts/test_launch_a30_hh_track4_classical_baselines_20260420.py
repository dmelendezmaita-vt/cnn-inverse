...
from launch_a30_hh_track4_classical_baselines_20260420 import build_env, build_srun_cmd
...
def main() -> None:
    matrix = (
        REPO
        / "data/important_notes/optimization_track_20260420_a30_hh_track4_classical_baselines/tables"
        / "a30_hh_track4_classical_baselines_matrix_20260420_a30_hh_track4_classical_baselines.csv"
    )
    row = load_row(matrix, "a30t4cls_0001")

    env = build_env(row, Path("/tmp/a30t4clstest"))
    if env.get("BASELINE_NAME") != "ridge_svd128_alpha1":
        raise AssertionError("expected ridge baseline")
    ...
    print("A30_HH_TRACK4_CLASSICAL_BASELINES_LAUNCH_TEST_OK")
