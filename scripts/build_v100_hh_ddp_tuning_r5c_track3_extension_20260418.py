...
MATRIX_CSV = TABLES / f"v100_hh_ddp_tuning_r5c_track3_extension_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_ddp_tuning_r5c_track3_extension_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants_by_track = {
        "track3_hh_reduced": [
            ...
            ("ddpopt_gabv1_b100", "Confirmed R5c track3 tune: gradient_as_bucket_view on, bucket_cap_mb=100.", None, "True", "100"),
        ],
    }
    ...
    for track_key, meta in track_meta.items():
        for seed in [974, 975, 976]:
            for variant_name, variant_desc, ddp_static_graph, ddp_gabv, ddp_bucket in variants_by_track[track_key]:
                ...
                updates = {
                    "description": f'"{track_key} {variant_name} phase R5c track3 extension {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        ...
                        "phase": "phaseR5c_ddp_tuning_track3_extension",
                        ...
                        "notes": "Track3-only seed expansion for the confirmed R5c training-speed winner.",
                    }
                )
    ...
    write_text(
        ...
        "\n".join(
            [
                f"# V100 HH DDP Tuning R5c Track3 Extension ({DATE_LABEL})",
                "",
                f"- matrix csv: `{MATRIX_CSV}`",
                f"- rows: `{len(rows)}`",
                "- scope: 1 track x 2 variants x 3 seeds",
                "",
                "This follow-on package stays focused on the only consistent training-speed signal:",
