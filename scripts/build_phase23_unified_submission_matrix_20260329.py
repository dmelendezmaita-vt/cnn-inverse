...
CONFIG_ROOT = REPO / f"pytorch/configs/reruns_{DATE_TAG}_phase23"
...
def main() -> None:
    ...
    for nodes, ntrain, gbs in weak_points:
        for seed in phase2_weak_extra_seeds:
            cfg_rel = (
                f"pytorch/configs/reruns_{DATE_TAG}_phase23/track2_true_weak/"
    ...
    for track_key, meta in track_meta.items():
        for lr, ep in lr_epoch_combos:
            ...
            for seed in phase2_lr_extra_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_phase23/{track_key}/phase2_lr/"
    ...
    for track_key, meta in track_meta.items():
        for sub_len in phase2_input_lengths:
            for seed in phase2_input_extra_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_phase23/{track_key}/phase2_input/"
    ...
    for track_key, meta in track_meta.items():
        for gbs in phase3_batch_sizes:
            for seed in phase3_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_phase23/{track_key}/phase3_batch/"
