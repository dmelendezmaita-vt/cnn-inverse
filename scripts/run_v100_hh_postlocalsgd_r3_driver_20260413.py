...
MATRIX_CSV = TABLES / f"v100_hh_postlocalsgd_r3_matrix_{DATE_TAG}.csv"
REGISTRY_CSV = TABLES / f"v100_hh_postlocalsgd_r3_registry_{DATE_TAG}.csv"
STATE_JSON = NOTES / f"v100_hh_postlocalsgd_r3_driver_state_{DATE_TAG}.json"
LAUNCHER = REPO / "scripts" / "launch_v100_hh_postlocalsgd_r3_20260413.py"
RECONCILE = REPO / "scripts" / "reconcile_v100_hh_postlocalsgd_r3_20260413.py"
SUMMARIZE = REPO / "scripts" / "summarize_v100_hh_postlocalsgd_r3_20260413.py"
...
def main() -> int:
    send_ntfy(
        "neuro-collab v100 R3 driver",
        "Started persistent R3 post-local SGD driver. It will keep launching incomplete rows in the separate 20260413 R3 package on the running 7-day V100 allocation 355055.",
    )
    ...
    while True:
        ...
        if row_id is None:
            send_ntfy(
                "neuro-collab v100 R3 driver",
                "All rows in the 20260413 V100 HH post-local SGD R3 package are now complete. The driver is exiting.",
            )
            return 0
        ...
        send_ntfy(
            "neuro-collab v100 R3 driver",
            f"Launching next incomplete R3 row {row_id} in running allocation {running['job_id']} on {nodelist}.",
        )
