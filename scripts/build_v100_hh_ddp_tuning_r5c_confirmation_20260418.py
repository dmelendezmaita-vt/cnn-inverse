...
DATE_TAG = "20260418_v100_hh_ddp_tuning_r5c_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_ddp_tuning_r5c_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_ddp_tuning_r5c_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    track_meta = {
        "track3_hh_reduced": {
            ...
            "prefix": "reduced_data",
            "shared_data_dir": str(
                IMPORTANT / "optimization_track_20260411_v100_interactive" / "shared_data" / "track3_hh_reduced" / "reduced_data"
            ),
        },
        "track4_hh_full": {
            "short": "t4",
    ...
    variants_by_track = {
        "track3_hh_reduced": [
            ("baseline", "Baseline R2 DDP settings.", None, None, None),
            ("ddpopt_gabv1_b100", "Winning seed-971 R5c tune for track3: gradient_as_bucket_view on, bucket_cap_mb=100.", None, "True", "100"),
        ],
        "track4_hh_full": [
            ("baseline", "Baseline R2 DDP settings.", None, None, None),
            ("ddpopt_gabv1_b25", "Winning seed-971 R5c tune for track4: gradient_as_bucket_view on, bucket_cap_mb=25.", None, "True", "25"),
        ],
    }
    ...
    for track_key, meta in track_meta.items():
        for seed in [972, 973]:
            for variant_name, variant_desc, ddp_static_graph, ddp_gabv, ddp_bucket in variants_by_track[track_key]:
                row_num += 1
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}/{track_key}/"
                    f"params_{meta['short']}_{variant_name}_n4_s{seed}.yaml"
                )
                updates = {
                    "description": f'"{track_key} {variant_name} phase R5c confirmation {DATE_LABEL}"',
                ...
                if ddp_static_graph is not None:
                    updates["ddp_static_graph"] = ddp_static_graph
                if ddp_gabv is not None:
                    updates["ddp_gradient_as_bucket_view"] = ddp_gabv
                if ddp_bucket is not None:
                    updates["ddp_bucket_cap_mb"] = ddp_bucket
                create_config(meta["base"], cfg_rel, updates)
                rows.append(
                    {
                        "row_id": f"v100r5c_{row_num:04d}",
                        "phase": "phaseR5c_ddp_tuning_confirmation",
                        ...
                        "strategy_description": variant_desc,
                        "params_file": cfg_rel,
                        "tar_path": meta["tar"],
                        ...
                        "save_diagnostic_plots": "False" if track_key == "track3_hh_reduced" else "True",
                        "save_checkpoints_epochs": "null" if track_key == "track3_hh_reduced" else "10",
                        "task_slug": f"{track_key}_{variant_name}_n4_s{seed}",
                        "notes": "Seed-expansion confirmation sweep for the winning seed-971 R5c DDP tuning settings.",
                    }
                )
    ...
    write_text(
        ...
        "\n".join(
            [
                f"# V100 HH DDP Tuning R5c Confirmation ({DATE_LABEL})",
                "",
                f"- matrix csv: `{MATRIX_CSV}`",
                f"- rows: `{len(rows)}`",
                "- scope: 2 tracks x 2 variants x 2 seeds",
                "",
                "Winning seed-971 variants carried forward:",
                "- `track3_hh_reduced`: baseline vs `ddpopt_gabv1_b100`",
                "- `track4_hh_full`: baseline vs `ddpopt_gabv1_b25`",
                "- seeds: `972`, `973`",
            ]
        )
        + "\n"
