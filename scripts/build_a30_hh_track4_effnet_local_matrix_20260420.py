...
DATE_TAG = "20260420_a30_hh_track4_effnet_local_matrix"
...
MATRIX_CSV = TABLES / f"a30_hh_track4_effnet_local_matrix_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a30_hh_track4_effnet_local_matrix_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    schedule_variants = {
        "e400": {"epochs": "400", "linear_epochs": "60", "constant_epochs": "60", "final_learning_rate": "7.5e-7"},
        "lowfloor": {"epochs": "400", "linear_epochs": "60", "constant_epochs": "40", "final_learning_rate": "1.0e-7"},
        "e500": {"epochs": "500", "linear_epochs": "75", "constant_epochs": "75", "final_learning_rate": "7.5e-7"},
    }
    variants = {}
    for beta_tag, beta_value in [("003", "0.03"), ("005", "0.05"), ("007", "0.07")]:
        for weight_tag, weight_mode in [("invstd", "inverse_std"), ("invvar", "inverse_var")]:
            for sched_tag, sched in schedule_variants.items():
                key = f"effnet_beta{beta_tag}_{weight_tag}_{sched_tag}"
                variants[key] = {
                    "description": f"Track4 A30 EfficientNet local matrix: beta={beta_value} {weight_mode} {sched_tag}.",
                    "type": '"EfficientNet"',
                    "dropout": "0.1",
                    "target_loss_weight_mode": weight_mode,
                    "loss_type": "huber",
                    "huber_beta": beta_value,
                    "learning_rate": "1.5e-4",
                    "init_learning_rate": "7.5e-6",
                    "epochs": sched["epochs"],
                    "linear_epochs": sched["linear_epochs"],
                    "constant_epochs": sched["constant_epochs"],
                    "final_learning_rate": sched["final_learning_rate"],
                }

    rows = []
    row_num = 0
    for seed in [1069, 1070]:
        variant_items = list(variants.items())
        for variant_idx, (strategy_id, variant) in enumerate(variant_items):
            row_num += 1
            block_id = 1 + (variant_idx // 3)
            group = f"track4_a30_effnet_local_matrix_seed_{seed}_block{block_id}"
            cfg_rel = (
                f"pytorch/configs/reruns_{DATE_TAG}/{track_key}/"
                f"params_{meta['short']}_{strategy_id}_n1_s{seed}.yaml"
            )
            updates = {
                "description": f'"{track_key} {strategy_id} a30 effnet local matrix {DATE_LABEL}"',
                "type": variant["type"],
            ...
            rows.append(
                {
                    "row_id": f"a30t4mat_{row_num:04d}",
                    "phase": "phaseAO_track4_a30_effnet_local_matrix",
                    "phase_label": "Track4 A30 EfficientNet local matrix",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_a30_effnet_local_matrix",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "A30-side local matrix over EfficientNet beta, weighting, and schedule family.",
                }
            )
    ...
    notes = [
        f"# A30 HH Track4 EfficientNet Local Matrix ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- run a local matrix over the narrowed EfficientNet beta/weighting/schedule neighborhood on A30",
        "",
        "## Variants",
        "- `avgpool_baseline`",
