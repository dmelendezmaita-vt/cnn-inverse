...
CONFIG_ROOT = REPO / f"pytorch/configs/reruns_{DATE_TAG}_optfix"
...
def main() -> None:
    ...
    tracks = {
        "track3_hh_reduced": TrackMeta(
            ...
            short="t3",
            base_cfg="pytorch/configs/third_track_hh/params_dnn_tar_hh.yaml",
        ...
        "track4_hh_full": TrackMeta(
            key="track4_hh_full",
            short="t4",
            base_cfg="pytorch/configs/fourth_track_hh_full/params_dnn_tar_hh_full.yaml",
    ...
    for track_key, meta in tracks.items():
        for ntrain in phaseA_ntrain[track_key]:
            for seed in phaseA_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_optfix/{track_key}/phaseA_recovery_ntrain/"
                ...
                add_row(
                    ...
                    concurrency_group=f"a30_mem{meta.mem_gib}",
            ...
            for sub_len in phaseB_sub_lengths:
                for nodes in phaseB_nodes:
                    for seed in phaseB_seeds:
                        ...
                        cfg_rel = (
                            f"pytorch/configs/reruns_{DATE_TAG}_optfix/{track_key}/phaseB_scaling_improve/"
                        ...
                        add_row(
                            ...
                            concurrency_group=f"a30_mem{meta.mem_gib}_n{nodes}",
    ...
    lines = [
        ...
        "- All new outputs are written under `data/important_notes/optimization_track_20260405/`.",
        "- New configs are written under `pytorch/configs/reruns_20260405_optfix/`.",
