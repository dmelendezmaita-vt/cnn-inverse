...
NOTES_MD = NOTES / f"v100_hh_track4_huber_beta_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "beta_0p1": {
            "description": "Track4 Huber beta retune: beta=0.1",
            "huber_beta": "0.1",
        },
        "beta_0p3": {
            "description": "Track4 Huber beta retune: beta=0.3",
            "huber_beta": "0.3",
        },
        "beta_1p0": {
            "description": "Track4 Huber beta retune baseline: beta=1.0",
            "huber_beta": "1.0",
        },
        "beta_3p0": {
            "description": "Track4 Huber beta retune: beta=3.0",
            "huber_beta": "3.0",
        },
    }
    ...
    row_num = 0
    for seed in [997, 998]:
        group = f"track4_seed{seed}_huber_beta"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} huber-beta retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4hb_{row_num:04d}",
                    "phase": "phaseQ_track4_huber_beta_retune",
                    "phase_label": "Track4 Huber beta retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_huber_beta_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Track4 Huber beta retune on the strongest current baseline branch.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Huber Beta Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether the remaining track4 tradeoff is a Huber-transition issue on the strongest baseline branch",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
