...
DATE_TAG = "20260419_v100_hh_track4_multitask_heads_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_multitask_heads_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_multitask_heads_retune_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    net_keys = {
        ...
        "conv_pool_padding",
        "activation_fn",
        "target_head_strategy",
        "head_dense_layer_sizes",
        "target_head_groups",
        "learn_task_uncertainty",
    }
    optimizer_keys = {
        "learning_rate",
        "beta1",
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in multitask-heads builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_shared": {
            "description": "Track4 multitask baseline: avg-pool shared head.",
            **base_arch,
            "loss_type": "huber",
            "target_loss_weight_mode": "inverse_std",
        },
        "avgpool_per_target_heads": {
            "description": "Track4 multitask retune: per-target heads on shared trunk.",
            **base_arch,
            "type": '"ConvMultiHead"',
            "target_head_strategy": '"per_target"',
            "head_dense_layer_sizes": "[64]",
            "loss_type": "huber",
            "target_loss_weight_mode": "inverse_std",
        },
        "avgpool_grouped_heads": {
            "description": "Track4 multitask retune: grouped heads on shared trunk.",
            **base_arch,
            "type": '"ConvMultiHead"',
        ...
        "avgpool_uncertainty_huber": {
            "description": "Track4 multitask retune: learned task uncertainty with shared head.",
            **base_arch,
            "learn_task_uncertainty": "True",
            "loss_type": "uncertainty_huber",
    ...
    rows = []
    row_num = 0
    for seed in [1015, 1016]:
        group = f"track4_seed{seed}_multitask_heads"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} multitask-heads retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4mth_{row_num:04d}",
                    "phase": "phaseY_track4_multitask_heads_retune",
                    "phase_label": "Track4 multitask heads retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_multitask_heads_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Multitask follow-up after stabilizing the track4 loss scale and structural baseline.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Multitask Heads Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- compare the stabilized track4 baseline against per-target heads, grouped heads, and learned task uncertainty",
        "",
        "## Variants",
        "- `avgpool_shared`",
        "- `avgpool_per_target_heads`",
