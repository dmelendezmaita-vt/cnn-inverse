...
REGISTRY_CSV = TABLES / f"v100_hh_scaling_redesign_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"v100_hh_scaling_redesign_run_log_{DATE_TAG}.json"
...
def resolve_allocation(args) -> tuple[str, str, list[str]]:
...
def launch_one(row, alloc_job_id, alloc_nodelist, node_slice, dry_run):
...
def launch_group(group_rows, alloc_job_id, alloc_nodelist, hosts, dry_run):
    ...
    if dry_run:
        return [launch_one(row, alloc_job_id, alloc_nodelist, node_slice, True) for row, node_slice in zip(group_rows, slices)]
    procs = []
    started_at = now_iso()
        ...
        results.append(
            {
                ...
                "strategy_id": row["strategy_id"], "track_key": row["track_key"], "policy": row["policy"], "nodes": row["nodes"], "seed": row["seed"],
                "status": "COMPLETED" if rc == 0 else "FAILED", "return_code": str(rc), "alloc_job_id": alloc_job_id, "alloc_nodelist": alloc_nodelist,
                "assigned_nodes": ",".join(node_slice), "started_at": started_at, "ended_at": now_iso(), "elapsed_sec": f"{max(0.0, time.time()-start_ts):.3f}",
                "run_output_root": str(run_output_root), "stdout_log": str(stdout_log), "stderr_log": str(stderr_log),
                "params_file": row["params_file"], "shared_data_dir": row["shared_data_dir"], "notes": "",
            }
        )
...
def main():
    ...
    for row in selected:
        grouped[row["launch_group"]].append(row)

    results = []
    for group_id, rows in sorted(grouped.items()):
        rows = sorted(rows, key=lambda r: r["row_id"])
        if len(rows) == 1:
            results.append(launch_one(rows[0], alloc_job_id, alloc_nodelist, [hosts_for_launch[0]], args.dry_run))
        else:
            results.extend(launch_group(rows, alloc_job_id, alloc_nodelist, hosts_for_launch, args.dry_run))
