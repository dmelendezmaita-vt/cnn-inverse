...
NOTES_MD = NOTES / f"a30_hh_track4_knn_targeted_tuning_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    baselines = {
        "knn_k3": "k-nearest-neighbors regression (k=3, distance weighted).",
        "knn_k5": "k-nearest-neighbors regression (k=5, distance weighted).",
        "knn_k7": "k-nearest-neighbors regression (k=7, distance weighted).",
        "knn_k9": "k-nearest-neighbors regression (k=9, distance weighted).",
        "knn_k11": "k-nearest-neighbors regression (k=11, distance weighted).",
    }

    rows = []
    row_num = 0
    for seed in [1091, 1092, 1093, 1094]:
        baseline_items = list(baselines.items())
        for idx, (baseline_name, baseline_desc) in enumerate(baseline_items):
            block_id = 1 + (idx // 4)
            group = f"track4_a30_knn_targeted_tuning_seed_{seed}_block{block_id}"
            row_num += 1
            cfg_rel = (
            ...
            rows.append(
                {
                    "row_id": f"a30t4knn_{row_num:04d}",
                    "phase": "phaseAP_track4_a30_knn_targeted_tuning",
                    "phase_label": "Track4 A30 knn targeted tuning",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_a30_knn_targeted_tuning",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "cpus_per_node": "64",
                    ...
                    "notes": "Fresh-seed kNN local search around the current robustness winner.",
                }
            )
    ...
    notes = [
        f"# A30 HH Track4 KNN Targeted Tuning ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- run a local kNN search around the current robustness winner on fresh seeds",
        "",
        "## Variants",
