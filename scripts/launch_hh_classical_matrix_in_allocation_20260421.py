...
DEFAULT_STEP_SCRIPT = REPO / "scripts" / "run_classical_baseline_step_20260420.sh"
...
def build_env(row: Dict[str, str], run_output_root: Path) -> Dict[str, str]:
    ...
    if row.get("feature_mode"):
        env["FEATURE_MODE"] = row["feature_mode"]
    return env
...
def launch_group(
    ...
    elif mode == "serial_1x1n":
        if not hosts:
            raise RuntimeError("serial_1x1n requires at least one available host")
        slices = [[hosts[0]] for _ in group_rows]
    else:
        raise RuntimeError(f"Unsupported launch_mode={mode}")
    ...
    for row, node_slice in zip(group_rows, slices):
        ...
        cmd = build_srun_cmd(cluster, alloc_job_id, node_slice, row, step_script)
        ...
        if mode == "serial_1x1n":
            ...
            procs.append((row, node_slice, proc, out_f, err_f, start_ts, run_output_root, stdout_log, stderr_log))
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch classical HH matrix rows inside an existing allocation.")
    ...
    if not hosts:
        raise SystemExit(f"No free hosts in allocation {alloc_job_id}.")
    ...
    if not pending_rows:
        print("All requested classical rows already completed.")
        return
