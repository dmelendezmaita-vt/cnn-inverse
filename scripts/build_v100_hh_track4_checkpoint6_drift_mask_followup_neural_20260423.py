...
DATE_TAG = "20260423_v100_hh_track4_checkpoint6_drift_mask_followup_neural"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_checkpoint6_drift_mask_followup_neural_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_checkpoint6_drift_mask_followup_neural_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in checkpoint6 drift/mask neural builder: {k}")
...
def main() -> None:
    ...
    shifts = [
        ...
        ("drift005", {
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Linear baseline drift, std=0.05.", "block1"),
        ("drift010", {
            "features_additive_noise_std": "0.0",
            "features_multiplicative_noise_std": "0.0",
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Linear baseline drift, std=0.10.", "block2"),
        ("drift020", {
            "features_additive_noise_std": "0.0",
            "features_multiplicative_noise_std": "0.0",
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Linear baseline drift, std=0.20.", "block2"),
        ("mask10", {
            "features_additive_noise_std": "0.0",
            "features_multiplicative_noise_std": "0.0",
            "features_baseline_drift_std": "0.0",
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Contiguous missing segment covering 10% of the trace.", "block3"),
        ("mask20", {
            "features_additive_noise_std": "0.0",
            "features_multiplicative_noise_std": "0.0",
            "features_baseline_drift_std": "0.0",
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Contiguous missing segment covering 20% of the trace.", "block3"),
        ("mask30", {
            "features_additive_noise_std": "0.0",
            "features_multiplicative_noise_std": "0.0",
            "features_baseline_drift_std": "0.0",
            ...
            "features_sub_length": "2000",
            "features_sub_step": "1",
        }, "Contiguous missing segment covering 30% of the trace.", "block4"),
    ]

    rows = []
    row_num = 0
    for shift_name, shift_cfg, shift_desc, block_name in shifts:
        launch_mode = "concurrent_8x1n" if block_name != "block4" else "concurrent_4x1n"
        group = f"track4_checkpoint6_drift_mask_{block_name}"
        for seed in [1071, 1072, 1073, 1074]:
            ...
            updates = {
                "description": f'"{track_key} {WINNER_STRATEGY} drift/mask {shift_name} {DATE_LABEL}"',
                "type": '"EfficientNet"',
                "dropout": "0.1",
            ...
            rows.append(
                {
                    "row_id": f"v100dmn_{row_num:04d}",
                    "phase": "phaseCP6_track4_drift_mask_followup_neural",
                    "phase_label": "Checkpoint6 drift/mask follow-up neural",
                    "launch_mode": launch_mode,
                    "launch_group": group,
                    "family": "track4_checkpoint6_drift_mask_followup_neural",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "1",
                    ...
                    "strategy_description": f"Focused drift/mask follow-up: {shift_desc}",
                    "params_file": cfg_rel,
                    "tar_path": meta["tar"],
                    ...
                    "notes": "Focused checkpoint6 drift/mask follow-up on Falcon V100.",
                }
            )
    ...
    note_lines = [
        f"# V100 HH Track4 Checkpoint6 Drift/Mask Follow-Up Neural ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- tighten the two unstable checkpoint6 shift families for the neural baseline only",
        "- determine whether the drift and masking flips persist or intensify with severity",
        "",
        "## Shift Ladder",
