...
DATE_TAG = "20260419_v100_hh_track4_efficientnet_objective_tuning_b"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_efficientnet_objective_tuning_b_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_efficientnet_objective_tuning_b_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in EfficientNet objective tuning builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 EfficientNet objective tuning baseline: avg-pool ConvNet winner.",
            "net_type": '"ConvNet"',
        ...
        "effnet_best": {
            "description": "Track4 EfficientNet best-so-far sidecar.",
            "net_type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
            "huber_beta": "0.1",
        ...
        "effnet_beta005": {
            "description": "Track4 EfficientNet objective tuning: lower huber beta.",
            "net_type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_uniform": {
            "description": "Track4 EfficientNet objective tuning: uniform target weighting.",
            "net_type": '"EfficientNet"',
            "dropout": "0.1",
    ...
    rows = []
    row_num = 0
    for seed in [1034, 1035]:
        group = f"track4_effnet_objective_seed_{seed}"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} efficientnet objective tuning {DATE_LABEL}"',
                "type": variant["net_type"],
            ...
            if variant.get("dropout") is not None:
                updates["dropout"] = variant["dropout"]
            create_config(meta["base"], cfg_rel, updates)
            rows.append(
                {
                    "row_id": f"v100t4efob_{row_num:04d}",
                    "phase": "phaseAE_track4_efficientnet_objective_tuning",
                    "phase_label": "Track4 EfficientNet objective tuning",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_efficientnet_objective_tuning",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Focused EfficientNet objective tuning to try to recover MAE without losing the R2/MSE gains.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 EfficientNet Objective Tuning B ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- try to recover MAE in the EfficientNet sidecar without giving back its R2/MSE gains",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `effnet_best`",
