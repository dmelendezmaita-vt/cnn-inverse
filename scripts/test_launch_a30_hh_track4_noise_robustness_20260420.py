...
from launch_a30_hh_track4_noise_robustness_20260420 import build_env, build_srun_cmd
...
def main() -> None:
    matrix = (
        REPO
        / "data/important_notes/optimization_track_20260420_a30_hh_track4_noise_robustness/tables"
        / "a30_hh_track4_noise_robustness_matrix_20260420_a30_hh_track4_noise_robustness.csv"
    )
    row = load_row(matrix, "a30t4rob_0001")

    env = build_env(row, "351880", "fal011", 29671, Path("/tmp/a30t4robtest"))
    if env.get("DATA_ACCESS_MODE") != "direct_tar":
        raise AssertionError("expected direct_tar access mode")
    if "EVAL_ONLY_CHECKPOINT" not in env or not env["EVAL_ONLY_CHECKPOINT"]:
        raise AssertionError("expected eval-only checkpoint in environment")
    ...
    print("A30_HH_TRACK4_NOISE_ROBUSTNESS_LAUNCH_TEST_OK")
