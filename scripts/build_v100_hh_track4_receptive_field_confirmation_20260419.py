...
DATE_TAG = "20260419_v100_hh_track4_receptive_field_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_receptive_field_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_receptive_field_confirmation_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in receptive-field builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "rf_base_beta0p1": {
            "description": "Track4 receptive-field baseline winner: beta=0.1, kernel3 stride2.",
        ...
        "rf_kernel7_beta0p1": {
            "description": "Track4 receptive-field retune: larger kernels (7) with stride2.",
            "conv_layer_sizes": "[8, 16, 32]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
            "conv_layer_kernel": "7",
            "conv_layer_stride": "2",
        ...
        "rf_avgpool_beta0p1": {
            "description": "Track4 receptive-field retune: stride1 convs with avg-pool downsampling.",
            "conv_layer_sizes": "[8, 16, 32]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
            "conv_layer_kernel": "3",
            "conv_layer_stride": "1",
        ...
        "rf_k7_wide_beta0p1": {
            "description": "Track4 receptive-field confirmation sidecar: wider kernel7 model.",
    ...
    rows = []
    row_num = 0
    for seed in [1005, 1006]:
        group = f"track4_seed{seed}_receptive_field_confirm"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} receptive-field confirmation {DATE_LABEL}"',
                ...
                "conv_layer_kernel": variant["conv_layer_kernel"],
                "conv_layer_stride": variant["conv_layer_stride"],
                "conv_pool_type": variant["conv_pool_type"],
                "conv_pool_kernel": variant["conv_pool_kernel"],
                "conv_pool_stride": variant["conv_pool_stride"],
                "conv_pool_padding": variant["conv_pool_padding"],
            }
            create_config(meta["base"], cfg_rel, updates)
            rows.append(
                {
                    "row_id": f"v100t4rfcf_{row_num:04d}",
                    "phase": "phaseU_track4_receptive_field_confirmation",
                    "phase_label": "Track4 receptive-field confirmation",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_receptive_field_confirmation",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Fresh-seed confirmation of receptive-field finalists around the beta=0.1 winner.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Receptive-Field Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the most promising receptive-field finalists on fresh seeds",
        "",
        "## Variants",
        "- `rf_base_beta0p1`",
        "- `rf_kernel7_beta0p1`",
