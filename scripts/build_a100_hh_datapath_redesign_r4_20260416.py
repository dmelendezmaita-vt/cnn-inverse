...
MATRIX_CSV = TABLES / f"a100_hh_datapath_redesign_r4_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a100_hh_datapath_redesign_r4_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = [
        {
            ...
            "strategy_description": "Best operational-cleanup 1n baseline rerun on A100 under the R4 data-path instrumentation path.",
            "base_rel_tpl": (
                "pytorch/configs/reruns_20260411_v100_interactive/phaseB_codepath_screen/"
                "S4_leanartifacts_amp_shareddata_nolosssync/{track_key}/"
                "params_{short}_strong_gbs128_n1_lr3p0em4_sub2000_s{seed}.yaml"
            ),
            "notes": "A100 R4 core: repeat the best 1n operational-cleanup recipe under the explicit shared-data path and new loader instrumentation.",
        },
        {
            "phase": "phaseR4_datapath_core",
            "family": "datapath_core_verification",
            ...
            "strategy_description": "Current 4n production-safe baseline from R2 rerun on A100 under the R4 data-path instrumentation path.",
            "base_rel_tpl": (
                "pytorch/configs/reruns_20260413_v100_hh_scaling_replication_r2/{track_key}/"
                "params_{short}_R1_accum2_nosync_n4_s{seed}.yaml"
            ),
            "notes": "A100 R4 core: repeat the current best 4n recipe from R2 to measure what the data-path change does to end-to-end elapsed on A100.",
        },
        {
            ...
            "strategy_description": "Optional track3-only R3 sidecar on A100 under the R4 data path.",
            "base_rel_tpl": (
                "pytorch/configs/reruns_20260413_v100_hh_postlocalsgd_r3_stable/{track_key}/"
            ...
            "notes": "Optional A100 R4 sidecar: rerun the most interesting track3 R3 candidate under the new data path, preserving recovered runtime defaults.",
        },
    ]
    ...
    for variant in variants:
        for track_key in variant["track_keys"]:
            ...
            for seed in [971, 972, 973]:
                ...
                updates = {
                    "description": f'"{track_key} {variant["strategy_id"]} datapath redesign R4 A100 {DATE_LABEL}"',
    ...
    notes = [
        f"# A100 HH Datapath Redesign R4 Matrix ({DATE_LABEL})",
        "",
        ...
        "",
        "## Design",
        "- This package uses the current pending 4-node A100 interactive allocation",
        "- To preserve comparability with the V100 R2/R4 logic, runs use 2 GPUs per node inside the 6-GPU allocation",
        ...
        "- New loader instrumentation in pytorch/data.py will expose source kind and timing breakdown",
        "",
        "## Totals",
        "- Core rows: 12",
