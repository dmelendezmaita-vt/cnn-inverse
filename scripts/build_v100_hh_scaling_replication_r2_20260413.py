...
NOTES_MD = NOTES / f"v100_hh_scaling_replication_r2_notes_{DATE_TAG}.md"
...
def main() -> None:
        ...
        for nodes in [2, 4]:
            for variant_name, accum_steps in variants:
                for seed in [971, 972, 973]:
                    ...
                    updates = {
                        "description": f'"{track_key} {variant_name} replication phase R2 {DATE_LABEL}"',
                    ...
                    rows.append(
                        {
                            "row_id": f"v100r2_{row_num:04d}",
                            "phase": "phaseR2_replication",
                            "launch_mode": "dedicated",
                            "launch_group": "",
                            "family": "comm_reduction_replication",
                            "track_key": track_key,
                            "policy": "strong",
                            "nodes": str(nodes),
                            "gpus_per_node": "2",
                            ...
                            "strategy_description": f"Phase R2 replication with grad_accum_steps={accum_steps}",
                            "params_file": cfg_rel,
                            "tar_path": meta["tar"],
                            ...
                            "notes": (
                                "Phase R2 multi-seed replication of the R1 scaling signal."
                                + (
    ...
    notes = [
        f"# V100 HH Scaling Replication Matrix ({DATE_LABEL})",
        "",
        ...
        "",
        "## Design",
        "- New package for Phase R2 replication, separate from the first redesign screen",
        "- Tracks: track3_hh_reduced, track4_hh_full",
        "- Nodes: 2 and 4",
        "- Strategies: B0_operational_baseline and R1_accum2_nosync",
        "- Seeds: 971, 972, 973",
        (
            "- Track3 4n R1_accum2_nosync rows default to the active R5c candidate"
