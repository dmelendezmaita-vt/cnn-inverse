...
NOTES_MD = NOTES / f"v100_hh_track4_ensemble_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 ensemble confirmation baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "effnet_beta005_invstd": {
            "description": "Track4 ensemble confirmation candidate: EfficientNet beta=0.05 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta01_invstd": {
            "description": "Track4 ensemble confirmation candidate: EfficientNet beta=0.1 inverse_std.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
            "target_loss_weight_mode": "inverse_std",
            "loss_type": "huber",
        ...
        "effnet_beta005_invvar": {
            "description": "Track4 ensemble confirmation candidate: EfficientNet beta=0.05 inverse_var.",
            "type": '"EfficientNet"',
            "dropout": "0.1",
    ...
    rows = []
    row_num = 0
    for seed in [1048, 1049, 1050, 1051]:
        group = f"track4_ensemble_seed_{seed}"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} ensemble confirmation {DATE_LABEL}"',
                "type": variant["type"],
            ...
            rows.append(
                {
                    "row_id": f"v100t4ens_{row_num:04d}",
                    "phase": "phaseAI_track4_ensemble_confirmation",
                    "phase_label": "Track4 ensemble confirmation",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_ensemble_confirmation",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Fresh-seed confirmation package for ConvNet/EfficientNet variants plus post-hoc ensemble analysis.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Ensemble Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- evaluate the strongest ConvNet and EfficientNet recipes on fresh seeds, then compare post-hoc ensembles built from their saved predictions",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `effnet_beta005_invstd`",
