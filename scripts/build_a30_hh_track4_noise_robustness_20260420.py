...
DATE_TAG = "20260420_a30_hh_track4_noise_robustness"
...
MATRIX_CSV = TABLES / f"a30_hh_track4_noise_robustness_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a30_hh_track4_noise_robustness_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in robustness builder: {k}")
...
def main() -> None:
    ...
    for noise_tag, noise_std in noise_levels:
        group = f"track4_a30_noise_robustness_{noise_tag}"
        for seed in [1071, 1072, 1073, 1074]:
            ...
            updates = {
                "description": f'"{track_key} {WINNER_STRATEGY} robustness {noise_tag} {DATE_LABEL}"',
                "type": '"EfficientNet"',
                "dropout": "0.1",
            ...
            rows.append(
                {
                    "row_id": f"a30t4rob_{row_num:04d}",
                    "phase": "phaseAP_track4_a30_noise_robustness",
                    "phase_label": "Track4 A30 noise robustness",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_a30_noise_robustness",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "strategy_description": f"Winner eval-only robustness sweep at additive noise std={noise_std}.",
                    "params_file": cfg_rel,
                    "tar_path": meta["tar"],
                    ...
                    "notes": "Eval-only additive-noise robustness sweep around the best confirmed Track4 EfficientNet winner.",
                }
            )
    ...
    notes = [
        f"# A30 HH Track4 Noise Robustness ({DATE_LABEL})",
        "",
        ...
        "",
        "## Purpose",
        "- evaluate the best confirmed Track4 single-model candidate under additive observation noise without retraining",
        "- keep the seed-specific winner checkpoints fixed and vary only the evaluation noise condition",
        "",
        "## Noise Levels",
        "- `0.00` (clean)",
        "- `0.01`",
