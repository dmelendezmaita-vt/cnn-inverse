...
CONFIG_ROOT = REPO / f"pytorch/configs/reruns_{DATE_TAG}_opttrack"
    ...
    for track_key, meta in track_meta.items():
        for lr in phase1_lrs:
            ...
            for seed in phase1_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_opttrack/{track_key}/phase1_stability/"
            ...
            for sbr in phase2_sub_rand:
                for sub_len in phase2_sub_len:
                    for seed in phase2_seeds:
                        cfg_rel = (
                            f"pytorch/configs/reruns_{DATE_TAG}_opttrack/{track_key}/phase2_features/"
    ...
    for track_key, meta in track_meta.items():
        for gbs in phase3_batches:
            for seed in phase3_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_opttrack/{track_key}/phase3_batch/"
    ...
    for track_key, meta in track_meta.items():
        for ntrain in phase4_ntrain:
            for seed in phase4_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_opttrack/{track_key}/phase4_ntrain/"
    ...
    for track_key, meta in track_meta.items():
        for var in phase5_variants:
            for seed in phase5_seeds:
                cfg_rel = (
                    f"pytorch/configs/reruns_{DATE_TAG}_opttrack/{track_key}/phase5_robustness/"
