...
DATE_TAG = "20260419_v100_hh_track4_huber_beta_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_huber_beta_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_huber_beta_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "confirm_beta_1p0": {
            "description": "Track4 Huber beta confirmation baseline: beta=1.0.",
            "huber_beta": "1.0",
        },
        "confirm_beta_0p1": {
            "description": "Track4 Huber beta confirmation candidate: beta=0.1.",
            "huber_beta": "0.1",
        },
    }
    ...
    for seeds in [(999, 1000), (1001, 1002)]:
        group = f"track4_beta_confirm_{seeds[0]}_{seeds[1]}"
        for seed in seeds:
            for strategy_id, variant in variants.items():
                ...
                updates = {
                    "description": f'"{track_key} {strategy_id} huber-beta confirmation {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        "row_id": f"v100t4hbconf_{row_num:04d}",
                        "phase": "phaseR_track4_huber_beta_confirmation",
                        "phase_label": "Track4 Huber beta confirmation",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track4_huber_beta_confirmation",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "gpus_per_node": "2",
                        ...
                        "notes": "Fresh-seed confirmation of the strong beta=0.1 signal.",
                    }
                )
    ...
    notes = [
        f"# V100 HH Track4 Huber Beta Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the strong `beta=0.1` signal on fresh seeds against the old `beta=1.0` baseline",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
