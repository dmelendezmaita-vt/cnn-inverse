...
REGISTRY_CSV = TABLES / f"a30_hh_track4_frontier_head_to_head_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_frontier_head_to_head_run_log_{DATE_TAG}.json"
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; frontier head-to-head expects concurrent_4x1n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 A30 frontier head-to-head rows inside an existing allocation.")
    ...
    if not pending_rows:
        print("All requested frontier head-to-head rows already completed.")
        return
