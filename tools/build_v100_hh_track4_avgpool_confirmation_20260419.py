...
DATE_TAG = "20260419_v100_hh_track4_avgpool_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_avgpool_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_avgpool_confirmation_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in avg-pool confirmation builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "base_beta0p1": {
            "description": "Track4 avg-pool confirmation baseline: beta=0.1, kernel3 stride2.",
        ...
        "kernel7_beta0p1": {
            "description": "Track4 avg-pool confirmation sidecar: kernel7 stride2.",
            "conv_layer_sizes": "[8, 16, 32]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
            "conv_layer_kernel": "7",
            "conv_layer_stride": "2",
        ...
        "avgpool_beta0p1": {
            "description": "Track4 avg-pool confirmation candidate: stride1 convs with avg-pool downsampling.",
            "conv_layer_sizes": "[8, 16, 32]",
            "dense_layer_sizes": "[128, 128, 128, 128]",
            "conv_layer_kernel": "3",
            "conv_layer_stride": "1",
    ...
    for seed in [1007, 1008, 1009, 1010]:
        group = f"track4_seed{seed}_avgpool_confirm"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} avg-pool confirmation {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4avgcf_{row_num:04d}",
                    "phase": "phaseW_track4_avgpool_confirmation",
                    "phase_label": "Track4 avg-pool confirmation",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_avgpool_confirmation",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Fresh-seed confirmation of the avg-pool candidate around the beta=0.1 winner.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Avg-Pool Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the strongest structural track4 candidate on fresh seeds",
        "",
        "## Variants",
        "- `base_beta0p1`",
        "- `kernel7_beta0p1`",
