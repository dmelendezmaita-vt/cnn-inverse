...
DATE_TAG = "20260419_v100_hh_track4_finalists_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_finalists_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_finalists_confirmation_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in finalists builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "base_standard": {
            "description": "Track4 finalists confirmation baseline standard model.",
        ...
        "base_wide": {
            "description": "Track4 finalists confirmation baseline wide model.",
            "learning_rate": "3.0e-4",
            "global_train_batch_size": "128",
            "train_batch_size": "128",
        ...
        "g64_standard": {
            "description": "Track4 finalists confirmation low-lr g64 standard model.",
        ...
        "g64_wide": {
            "description": "Track4 finalists confirmation low-lr g64 wide model.",
            "learning_rate": "1.5e-4",
            "global_train_batch_size": "64",
            "train_batch_size": "64",
            "conv_layer_sizes": "[16, 32, 64]",
    ...
    row_num = 0
    for seed in [987, 988]:
        group = f"track4_seed{seed}_finalists"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} finalists confirmation {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4fin_{row_num:04d}",
                    "phase": "phaseL_track4_finalists_confirmation",
                    "phase_label": "Track4 finalists confirmation",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_finalists_confirmation",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Fresh-seed confirmation of the most plausible track4 finalists.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Finalists Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- compare the most plausible remaining track4 finalists on fresh seeds",
        "- avoid another fragmented sequence of one-off follow-up packages",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
