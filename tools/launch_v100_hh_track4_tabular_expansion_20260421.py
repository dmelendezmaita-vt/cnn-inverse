...
REGISTRY_CSV = TABLES / f"v100_hh_track4_tabular_expansion_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"v100_hh_track4_tabular_expansion_run_log_{DATE_TAG}.json"
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; tabular expansion expects concurrent_4x1n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 V100 tabular expansion rows inside an existing V100 allocation.")
    ...
    if not pending_rows:
        print("All requested tabular expansion rows already completed.")
        return
