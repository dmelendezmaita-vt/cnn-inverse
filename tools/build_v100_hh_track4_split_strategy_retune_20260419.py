...
DATE_TAG = "20260419_v100_hh_track4_split_strategy_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_split_strategy_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_split_strategy_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "seq_base": {
            "description": "Track4 split-strategy baseline: sequential split, standard eval window.",
            "split_strategy": "sequential",
            "features_sub_begin_random_eval": "False",
        },
        "rand_base": {
            "description": "Track4 split-strategy retune: random split, standard eval window.",
            "split_strategy": "random",
            "features_sub_begin_random_eval": "False",
        },
        "seq_randomeval": {
            "description": "Track4 split-strategy retune: sequential split, random eval window.",
            "split_strategy": "sequential",
            "features_sub_begin_random_eval": "True",
        },
        "rand_randomeval": {
            "description": "Track4 split-strategy retune: random split, random eval window.",
            "split_strategy": "random",
            "features_sub_begin_random_eval": "True",
        },
    }
    ...
    row_num = 0
    for seed in [995, 996]:
        group = f"track4_seed{seed}_split_strategy"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} split-strategy retune {DATE_LABEL}"',
                ...
                "features_sub_begin_random_eval": variant["features_sub_begin_random_eval"],
            ...
            rows.append(
                {
                    "row_id": f"v100t4sp_{row_num:04d}",
                    "phase": "phaseP_track4_split_strategy_retune",
                    "phase_label": "Track4 split strategy retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_split_strategy_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Track4 split-strategy retune around the strongest current baseline branch.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Split-Strategy Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether split strategy or eval-window alignment resolves the remaining track4 tradeoff on the strongest baseline branch",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
