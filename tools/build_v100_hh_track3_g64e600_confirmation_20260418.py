...
NOTES_MD = NOTES / f"v100_hh_track3_g64e600_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "confirm_base_g128_e400": {
            "description": "Track3 confirmation baseline: gbs128 epochs400.",
        ...
        "confirm_g64_e600": {
            "description": "Track3 confirmation candidate: gbs64 epochs600.",
            "learning_rate": "3.0e-4",
            "epochs": "600",
            "init_learning_rate": "1.5e-5",
            "linear_epochs": "60",
    ...
    rows = []
    row_num = 0
    # Two 4x1n groups: (974,975) and (976,977), with baseline + candidate per seed.
    for seeds in [(974, 975), (976, 977)]:
        group = f"track3_confirm_{seeds[0]}_{seeds[1]}"
        for seed in seeds:
            for strategy_id, variant in variants.items():
                ...
                rows.append(
                    {
                        "row_id": f"v100t3cf_{row_num:04d}",
                        "phase": "phaseE_track3_g64e600_confirmation",
                        "phase_label": "Track3 g64/e600 confirmation",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track3_g64e600_confirmation",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "gpus_per_node": "2",
                        ...
                        "notes": "Focused confirmation of the track3 g64/e600 win on fresh seeds.",
                    }
                )
    ...
    notes = [
        f"# V100 HH Track3 g64/e600 Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the track3 g64/e600 accuracy win on fresh seeds",
        "- compare only baseline g128/e400 vs candidate g64/e600",
        "- use seeds 974-977 for a clean follow-up",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
