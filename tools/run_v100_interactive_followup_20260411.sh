...
if [[ "${REBUILD_MATRIX}" == "1" || ! -f "${REPO}/data/important_notes/optimization_track_20260411_v100_interactive/tables/v100_interactive_followup_matrix_20260411_v100_interactive.csv" ]]; then
  python tools/build_v100_interactive_followup_matrix_20260411.py
fi
...
run_phase() {
  ...
  python tools/launch_v100_interactive_followup_20260411.py --phase "${phase}" "${LAUNCH_ARGS[@]}"
}
...
case "${PHASE_SET}" in
  phaseA)
    ...
    ;;
  phaseB)
    bash tools/prepare_v100_interactive_shared_data_20260411.sh
...
python tools/summarize_v100_interactive_followup_20260411.py
