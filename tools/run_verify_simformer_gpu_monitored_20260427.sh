...
/projects/neuro-collab/conda/neuro-collab-env/bin/python \
  tools/run_hh_track4_monitored_command_20260427.py \
  --resource-dir "$RESOURCE_DIR" \
  --gpu-expected 1 \
  --command "cd $REPO && env PYTHONPATH=\"$REPO:${REPO}/vendor/dlkit:\${PYTHONPATH:-}\" LD_LIBRARY_PATH=\"$SIM_LIB_PATHS:\${LD_LIBRARY_PATH:-}\" JAX_PLATFORMS=cuda $VENV_PY tools/verify_simformer_jax_gpu_20260427.py --size 4096 --repeats 8"
