...
REGISTRY_CSV = TABLES / f"a30_hh_track4_noise_robustness_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_noise_robustness_run_log_{DATE_TAG}.json"
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; robustness package expects concurrent_4x1n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 A30 noise robustness eval-only rows inside an existing A30 allocation.")
    ...
    if not hosts:
        raise SystemExit(f"Need at least 1 free host in allocation {alloc_job_id}, found 0 free: {hosts}")
    ...
    if not pending_rows:
        print("All requested robustness rows already completed.")
        return
