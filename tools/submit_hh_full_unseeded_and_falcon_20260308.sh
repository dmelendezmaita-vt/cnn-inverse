...
# Submit HH_full (fourth track) unseeded matrix on Tinkercliffs and both seeded/unseeded on Falcon.
...
PARAMS_FILE_HH="src/pytorch/configs/fourth_track_hh_full/params_dnn_tar_hh_full.yaml"
...
TRAIN_CSV_TC="${REPO}/data/important_notes/fourth_track_hh_full/hh_full_training_jobs.csv"
TEST_CSV_TC="${REPO}/data/important_notes/fourth_track_hh_full/hh_full_testing_jobs.csv"
SEED_CSV_TC="${REPO}/data/important_notes/fourth_track_hh_full/hh_full_seeded_training_jobs.csv"
SEED_TEST_CSV_TC="${REPO}/data/important_notes/fourth_track_hh_full/hh_full_seeded_testing_jobs.csv"
...
if [[ "${TARGET}" == "ALL" || "${TARGET}" == "TC" ]]; then
  ...
  for row in "${tc_jobs_train[@]}"; do
    ...
    job_name="trn_HH_full_${gpu}_${nodes}n${local_suffix}"
    node_type="${node_type}"
  ...
  for row in "${tc_jobs_test[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token <<< "${row}"
    local_suffix="$(tc_suffix "${node_type}")"
    job_name="tst_HH_full_${gpu}_${nodes}n${local_suffix}"
    node_type="${node_type}"
  ...
  for row in "${tc_train_seeded[@]}"; do
    ...
    local_suffix="$(tc_suffix "${node_type}")"
    job_name="trn_HH_full_${gpu}_${nodes}n${local_suffix}_s${seed}"
    node_type="${node_type}"
  ...
  for row in "${tc_test_seeded[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token seed <<< "${row}"
    local_suffix="$(tc_suffix "${node_type}")"
    job_name="tst_HH_full_${gpu}_${nodes}n${local_suffix}_s${seed}"
    node_type="${node_type}"
...
if [[ "${TARGET}" == "ALL" || "${TARGET}" == "FAL" ]]; then
  for row in "${fal_train_unseed[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token <<< "${row}"
    job_name="trn_HH_full_${gpu}_${nodes}n"
    node_type="${node_type}"
  ...
  for row in "${fal_test_unseed[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token <<< "${row}"
    job_name="tst_HH_full_${gpu}_${nodes}n"
    node_type="${node_type}"
  ...
  for row in "${fal_train_seeded[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token seed <<< "${row}"
    job_name="trn_HH_full_${gpu}_${nodes}n_s${seed}"
    node_type="${node_type}"
  ...
  for row in "${fal_test_seeded[@]}"; do
    IFS='|' read -r gpu node_type part qos nodes gpn cpus mem wall token seed <<< "${row}"
    job_name="tst_HH_full_${gpu}_${nodes}n_s${seed}"
    node_type="${node_type}"
...
echo "HH_full Falcon+TC unseeded/seeded submission complete (mode=${MODE}, target=${TARGET})."
