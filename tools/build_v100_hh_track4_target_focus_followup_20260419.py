...
DATE_TAG = "20260419_v100_hh_track4_target_focus_followup"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_target_focus_followup_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_target_focus_followup_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in target-focus builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_inverse_std": {
            "description": "Track4 target-focus baseline: avg-pool + inverse_std weighting.",
            "target_loss_weight_mode": "inverse_std",
        },
        "avgpool_manual_14_soft": {
            "description": "Track4 target-focus: softly emphasize hh_param_1 and hh_param_4.",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.25, 1.0, 1.0, 1.25, 1.0, 1.0]",
            "target_loss_weights_normalize": "True",
        },
        "avgpool_manual_14_hard": {
            "description": "Track4 target-focus: strongly emphasize hh_param_1 and hh_param_4.",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.5, 1.0, 1.0, 1.5, 1.0, 1.0]",
            "target_loss_weights_normalize": "True",
        },
    }
    ...
    rows = []
    row_num = 0
    for seed in [1017, 1018, 1019, 1020]:
        group = f"track4_seed{seed}_target_focus"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} target-focus follow-up {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4tf_{row_num:04d}",
                    "phase": "phaseZ_track4_target_focus_followup",
                    "phase_label": "Track4 target-focus follow-up",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_target_focus_followup",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Target-level follow-up after the stabilized track4 avg-pool winner.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Target-Focus Follow-Up ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether modest manual emphasis on the hardest remaining HH targets helps the stabilized track4 winner",
        "",
        "## Variants",
        "- `avgpool_inverse_std`",
        "- `avgpool_manual_14_soft`",
