...
REGISTRY_CSV = TABLES / f"a30_hh_track4_gpu_profile_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_gpu_profile_run_log_{DATE_TAG}.json"
STEP_SCRIPT = REPO / "tools" / "run_interactive_step_with_gpu_monitor_20260420.sh"
...
def build_srun_cmd(
    ...
    cmd.extend(
        [
            ...
            f"--gres=gpu:a30:{row['gpus_per_node']}",
            "--kill-on-bad-exit=1",
            "bash",
            str(STEP_SCRIPT),
        ]
...
def launch_group(
    ...
    if mode == "concurrent_4x1n":
        if not 1 <= len(group_rows) <= 4 or len(hosts) < len(group_rows):
            raise RuntimeError(f"{mode} requires 1 to 4 rows and at least 1 host per row")
        slices = [[hosts[i]] for i in range(len(group_rows))]
    elif mode == "concurrent_3x1n":
        if not 1 <= len(group_rows) <= 3 or len(hosts) < len(group_rows):
            raise RuntimeError(f"{mode} requires 1 to 3 rows and at least 1 host per row")
        slices = [[hosts[i]] for i in range(len(group_rows))]
    elif mode == "concurrent_2x2n":
        if not 1 <= len(group_rows) <= 2 or len(hosts) < 2 * len(group_rows):
            raise RuntimeError(f"{mode} requires 1 to 2 rows and 2 hosts per row")
        slices = [list(hosts[2 * i : 2 * (i + 1)]) for i in range(len(group_rows))]
    else:
