...
DATE_TAG = "20260419_v100_hh_track4_efficientnet_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_efficientnet_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_efficientnet_confirmation_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in efficientnet confirmation builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 EfficientNet confirmation baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "efficientnet_b0": {
            "description": "Track4 EfficientNet confirmation candidate.",
            "type": '"EfficientNet"',
            "dropout": "0.2",
        },
    }
    ...
    rows = []
    row_num = 0
    for seed_pair in [(1028, 1029), (1030, 1031)]:
        group = f"track4_effnet_confirm_{seed_pair[0]}_{seed_pair[1]}"
        for seed in seed_pair:
            for strategy_id, variant in variants.items():
                ...
                updates = {
                    "description": f'"{track_key} {strategy_id} efficientnet confirmation {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        "row_id": f"v100t4efcf_{row_num:04d}",
                        "phase": "phaseAC_track4_efficientnet_confirmation",
                        "phase_label": "Track4 EfficientNet confirmation",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track4_efficientnet_confirmation",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "gpus_per_node": "2",
                        ...
                        "notes": "Fresh-seed confirmation of EfficientNet against the current track4 winner.",
                    }
                )
    ...
    notes = [
        f"# V100 HH Track4 EfficientNet Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the strongest truly new backbone candidate against the current track4 winner",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `efficientnet_b0`",
