...
DATE_TAG = "20260421_a30_hh_track4_knn_robustness_tuning"
...
MATRIX_CSV = TABLES / f"a30_hh_track4_knn_robustness_tuning_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"a30_hh_track4_knn_robustness_tuning_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    baselines = {
        "knn_k5": "k-nearest-neighbors regression (k=5, current robustness anchor).",
        "knn_k7": "k-nearest-neighbors regression (k=7).",
        "knn_k9": "k-nearest-neighbors regression (k=9).",
        "knn_k11": "k-nearest-neighbors regression (k=11, strongest clean-data tuned kNN).",
        "knn_k13": "k-nearest-neighbors regression (k=13).",
    }
    noise_levels = {
        "clean": 0.0,
        "noise005": 0.05,
    ...
    rows = []
    row_num = 0
    for seed in [1095, 1096, 1097, 1098]:
        for noise_tag, noise_std in noise_levels.items():
            baseline_items = list(baselines.items())
            for idx, (baseline_name, baseline_desc) in enumerate(baseline_items):
                block_id = 1 + (idx // 4)
                group = f"track4_a30_knn_robustness_tuning_seed_{seed}_{noise_tag}_block{block_id}"
                row_num += 1
                cfg_rel = (
                ...
                rows.append(
                    {
                        "row_id": f"a30t4knnr_{row_num:04d}",
                        "phase": "phaseAP_track4_a30_knn_robustness_tuning",
                        "phase_label": "Track4 A30 kNN robustness tuning",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track4_a30_knn_robustness_tuning",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "cpus_per_node": "64",
                        ...
                        "notes": "Targeted kNN robustness sweep on the HH inverse task under additive feature noise with fresh seeds.",
                    }
                )
    ...
    notes = [
        f"# A30 HH Track4 KNN Robustness Tuning ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- run a local kNN robustness search around the current robustness winner under additive-noise conditions",
        "",
        "## Baselines",
        "- `knn_k5`",
        "- `knn_k7`",
