...
if [[ -x /home/dmm96/venvs/bayesflow_20260425_py312/bin/python ]]; then
  srun --jobid="${ALLOC_JOB_ID}" --exclusive --mem=0 -N1 -n1 -c6 -w "${HOST}" bash -lc \
    "FRAMEWORK_ENV_PATH=/home/dmm96/venvs/bayesflow_20260425_py312 FRAMEWORK_KIND=bayesflow SAVE_DIR=${RUN_ROOT}/bayesflow_v100_20260425 bash ${REPO}/tools/run_v100_hh_track4_framework_checks_20260425.sh"
fi

if [[ -x /home/dmm96/venvs/swyft_20260425/bin/python ]]; then
  srun --jobid="${ALLOC_JOB_ID}" --exclusive --mem=0 -N1 -n1 -c6 -w "${HOST}" bash -lc \
    "FRAMEWORK_ENV_PATH=/home/dmm96/venvs/swyft_20260425 FRAMEWORK_KIND=swyft SAVE_DIR=${RUN_ROOT}/swyft_v100_20260425 bash ${REPO}/tools/run_v100_hh_track4_framework_checks_20260425.sh"
fi
