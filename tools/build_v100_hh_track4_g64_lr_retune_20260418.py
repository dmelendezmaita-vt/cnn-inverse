...
DATE_TAG = "20260418_v100_hh_track4_g64_lr_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_g64_lr_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_g64_lr_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "g64_lr3e4_w40c40": {
            "description": "Track4 g64 retune baseline: lr3e-4 warmup40 constant40.",
        ...
        "g64_lr2e4_w40c40": {
            "description": "Track4 g64 retune: lower lr2e-4 warmup40 constant40.",
        ...
        "g64_lr2e4_w60c60": {
            "description": "Track4 g64 retune: lower lr2e-4 with longer warmup/sustain.",
            "learning_rate": "2.0e-4",
            "init_learning_rate": "1.0e-5",
            "linear_epochs": "60",
            "constant_epochs": "60",
        ...
        "g64_lr1p5e4_w60c60": {
            "description": "Track4 g64 retune: conservative lr1.5e-4 with longer warmup/sustain.",
    ...
    row_num = 0
    for seed in [973, 974]:
        group = f"track4_seed{seed}_g64_lr_retune"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} g64 lr retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4lr_{row_num:04d}",
                    "phase": "phaseF_track4_g64_lr_retune",
                    "phase_label": "Track4 g64 LR/schedule retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_g64_lr_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Focused LR/schedule retune around the track4 g64 branch.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 g64 LR Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- retune the track4 g64 branch after the batch/epoch confirmation showed that g64 changed R2 but did not improve pointwise error",
        "- target the scheduler/learning-rate shape instead of changing epochs again",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
