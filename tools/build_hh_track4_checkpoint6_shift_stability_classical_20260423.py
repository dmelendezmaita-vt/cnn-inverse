...
DATE_TAG = "20260423_hh_track4_checkpoint6_shift_stability_classical"
...
MATRIX_CSV = TABLES / f"hh_track4_checkpoint6_shift_stability_classical_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"hh_track4_checkpoint6_shift_stability_classical_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    baselines = {
        "random_forest_500": "Promoted clean-data classical winner under the richer observation representation.",
        "knn_k11": "Robustness winner under the richer observation representation.",
    }
    ...
    for baseline_name, baseline_desc in baselines.items():
        group = f"track4_checkpoint6_shift_{baseline_name}"
        for shift_name, seed, shift_cfg, shift_desc in shifts:
            ...
            rows.append(
                {
                    "row_id": f"cp6cls_{row_num:04d}",
                    "phase": "phaseCP6_track4_shift_stability_classical",
                    "phase_label": "Checkpoint6 classical shift stability",
                    "launch_mode": "serial_1x1n",
                    "launch_group": group,
                    "family": "track4_checkpoint6_shift_stability_classical",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "cpus_per_node": "18",
                    ...
                    "strategy_description": f"{baseline_desc} Shift family: {shift_desc}",
                    ...
                    "notes": "Checkpoint 6 classical shift-stability block for the clean-data and robustness winners.",
                }
            )
    ...
    note_lines = [
        f"# HH Track4 Checkpoint6 Classical Shift Stability ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- compare the current clean-data classical winner and the robustness winner under the main realistic shift families",
        "- keep the observation representation fixed at `raw_plus_fft256_summary12` while testing shift stability",
        "",
        "## Baselines",
