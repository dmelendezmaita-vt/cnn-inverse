...
CONFIG_DIR="${REPO}/src/pytorch/configs/track1_benchmark_v100"
DATA_DIR="${REPO}/data/2020-12-09"
CSV="${REPO}/data/important_notes/first_track_paper_parity/v100_track1_benchmark_training_jobs.csv"
RUN_ROOT="/projects/neuro-collab/data/runs/track1_benchmark_v100"
...
FIRST_HOST="$(scontrol show hostnames "${NODELIST}" | head -n 1)"
if [[ -z "${FIRST_HOST}" ]]; then
  echo "ERROR: no host found for allocation ${ALLOCATION_JOB_ID}" >&2
  exit 1
fi
...
for seed in ${SEEDS_STR}; do
  PARAMS_FILE="${CONFIG_DIR}/params_dnn_paper_behavior_seed${seed}.yaml"
  if [[ ! -f "${PARAMS_FILE}" ]]; then
    echo "ERROR: missing params file ${PARAMS_FILE}" >&2
    exit 1
  fi

  JOB_NAME="bench_V100_1g_s${seed}"
  ...
  NOTE="allocation_job_id=${ALLOCATION_JOB_ID}; parity_mode=single_gpu_paper_behavior; host=${FIRST_HOST}"
  printf '%s\n' \
    "track1_benchmark_v100,Falcon,V100,${JOB_NAME},${seed},1,1,${CPUS_PER_TASK},NA,${REQUESTED_TIME_LIMIT},${QOS},${PARTITION},${PARAMS_FILE},shared_fs,${ALLOCATION_JOB_ID},${STATUS},${elapsed_hms},${START_LOCAL},${RUN_DIR},${TRAIN_LAST},${VAL_LAST},${TRAIN_BEST},${VAL_BEST},${NOTE}" \
    >> "${CSV}"
