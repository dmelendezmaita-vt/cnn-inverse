...
def main() -> None:
    ...
    for idx, (strategy_id, model_name, hidden_features, num_transforms, n_train) in enumerate(variants, start=1):
        cfg_rel = (
            f"src/pytorch/configs/reruns_{DATE_TAG}/track4_hh_full/"
            f"params_t4cp2large_{strategy_id}_s{seed}.yaml"
        )
        create_config("src/pytorch/configs/fourth_track_hh_full/params_dnn_tar_hh_full.yaml", cfg_rel, seed=seed, n_train=n_train)
        rows.append(
            {
