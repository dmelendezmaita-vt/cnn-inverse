...
DATE_TAG = "20260419_v100_hh_track4_regularization_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_regularization_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_regularization_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "adamw_wd1e2": {
            "description": "Track4 regularization retune baseline wd=0.01.",
            "optimizer_type": "'AdamW'",
            "learning_rate": "3.0e-4",
            "weight_decay": "0.01",
        },
        "adamw_wd1e3": {
            "description": "Track4 regularization retune AdamW wd=0.001.",
            "optimizer_type": "'AdamW'",
            "learning_rate": "3.0e-4",
            "weight_decay": "0.001",
        },
        "adamw_wd0": {
            "description": "Track4 regularization retune AdamW wd=0.0.",
            "optimizer_type": "'AdamW'",
            "learning_rate": "3.0e-4",
            "weight_decay": "0.0",
        },
        "adam_wd0": {
            "description": "Track4 regularization retune Adam wd=0.0.",
            "optimizer_type": "'Adam'",
            "learning_rate": "3.0e-4",
            "weight_decay": "0.0",
        },
    ...
    row_num = 0
    for seed in [993, 994]:
        group = f"track4_seed{seed}_regularization"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} regularization retune {DATE_LABEL}"',
                ...
                "optimizer.type": variant["optimizer_type"],
                "weight_decay": variant["weight_decay"],
            }
            create_config(meta["base"], cfg_rel, updates)
            rows.append(
                {
                    "row_id": f"v100t4reg_{row_num:04d}",
                    "phase": "phaseO_track4_regularization_retune",
                    "phase_label": "Track4 regularization retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_regularization_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Track4 baseline-branch regularization retune under Huber + inverse_std.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Regularization Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether the remaining track4 tradeoff is a regularization / optimizer issue on the strongest current baseline branch",
        "- compare AdamW weight decay levels and Adam wd=0 on fresh seeds",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
