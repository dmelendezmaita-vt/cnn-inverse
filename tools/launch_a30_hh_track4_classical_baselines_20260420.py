...
REGISTRY_CSV = TABLES / f"a30_hh_track4_classical_baselines_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_classical_baselines_run_log_{DATE_TAG}.json"
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; classical package expects concurrent_4x1n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 A30 classical baseline rows inside an existing A30 allocation.")
    ...
    if len(hosts) < 3:
        raise SystemExit(f"Need 3 free hosts in allocation {alloc_job_id}, found {len(hosts)} free: {hosts}")
    ...
    if not pending_rows:
        print("All requested classical baseline rows already completed.")
        return
