...
case "${FRAMEWORK_KIND}" in
  bayesflow)
    python tools/run_hh_track4_aligned_multicurrent_bayesflow_20260425.py \
      --params src/pytorch/configs/fourth_track_hh_full/params_dnn_tar_hh_full.yaml \
      --save-dir "${SAVE_DIR}"
    ;;
  swyft)
    python tools/run_hh_track4_aligned_multicurrent_swyft_20260425.py \
      --params src/pytorch/configs/fourth_track_hh_full/params_dnn_tar_hh_full.yaml \
      --save-dir "${SAVE_DIR}"
    ;;
  *)
