...
from launch_v100_hh_track4_frontier_crossover_20260421 import build_env, build_srun_cmd
...
def main() -> None:
    matrix = (
        REPO
        / "data/important_notes/optimization_track_20260421_v100_hh_track4_frontier_crossover/tables"
        / "v100_hh_track4_frontier_crossover_matrix_20260421_v100_hh_track4_frontier_crossover.csv"
    )
    row = load_row(matrix, "v100t4xov_0001")

    env = build_env(row, Path("/tmp/v100t4xovtest"))
    ...
    cmd = build_srun_cmd("falcon", "368960", ["fal117"], row, cpus_per_task_override=24)
    assert_flag(cmd, "-M")
    assert_flag(cmd, "falcon")
    assert_flag(cmd, "--ntasks=1")
    assert_flag(cmd, "--cpus-per-task=24")
    print("V100_HH_TRACK4_FRONTIER_CROSSOVER_LAUNCH_TEST_OK")
