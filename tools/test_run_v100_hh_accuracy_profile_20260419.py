...
from run_v100_hh_accuracy_profile_20260419 import build_launch_cmd
...
def main() -> None:
    cmd = build_launch_cmd(
        profile="track3_accuracy_default",
        alloc_job_id="355055",
    ...
    assert_in(cmd, "track3_accuracy_default_g64_e600")
    assert_in(cmd, "--dry-run")
    assert_not_in(cmd, "--force-rerun")

    cmd = build_launch_cmd(
        profile="all_accuracy_defaults",
        alloc_job_id=None,
        alloc_nodelist=None,
        dry_run=False,
        force_rerun=True,
    ...
    assert_in(cmd, "track3_accuracy_default_g64_e600")
    assert_in(cmd, "track4_accuracy_default_beta0p1_avgpool")
    assert_in(cmd, "--force-rerun")

    print("V100_HH_ACCURACY_PROFILE_WRAPPER_TEST_OK")
