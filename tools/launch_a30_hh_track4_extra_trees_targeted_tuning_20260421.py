...
REGISTRY_CSV = TABLES / f"a30_hh_track4_extra_trees_targeted_tuning_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_extra_trees_targeted_tuning_run_log_{DATE_TAG}.json"
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; extra-trees tuning package expects concurrent_4x1n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 A30 Extra Trees targeted tuning rows inside an existing A30 allocation.")
    ...
    if not pending_rows:
        print("All requested extra-trees tuning rows already completed.")
        return
