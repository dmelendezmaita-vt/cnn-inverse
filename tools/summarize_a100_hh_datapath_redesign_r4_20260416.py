...
SUMMARY_JSON = NOTES / f"a100_hh_datapath_redesign_r4_summary_{DATE_TAG}.json"
SUMMARY_MD = NOTES / f"a100_hh_datapath_redesign_r4_summary_{DATE_TAG}.md"
...
def main():
    ...
    for row in completed:
        ...
        train_sec = eval_sec = data_load_sec = None
        if info_log.exists():
            ...
            md = re.search(r"data load timings \(sec\): total=([0-9.eE+-]+)", txt)
            if mt:
                train_sec = float(mt.group(1))
            if me:
                eval_sec = float(me.group(1))
            if md:
                data_load_sec = float(md.group(1))
    ...
    md = [
        f"# A100 HH Datapath Redesign R4 Summary ({DATE_TAG})",
        "",
        f"- registry: `{REGISTRY_CSV}`",
        f"- status counts: `{dict(state_counts)}`",
        f"- completed rows: `{len(completed)}`",
        "",
        "| phase | strategy | track | policy | nodes | completed | median elapsed (s) | median data-load (s) | median train (s) | median eval (s) | median test MSE | median test R2 |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
