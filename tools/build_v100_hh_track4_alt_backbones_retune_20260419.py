...
DATE_TAG = "20260419_v100_hh_track4_alt_backbones_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_alt_backbones_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_alt_backbones_retune_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in alt-backbones builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "avgpool_baseline": {
            "description": "Track4 alternate-backbone baseline: avg-pool ConvNet winner.",
            "type": '"ConvNet"',
        ...
        "efficientnet_b0": {
            "description": "Track4 alternate-backbone retune: EfficientNet1D.",
            "type": '"EfficientNet"',
            "dropout": "0.2",
        },
        "transformer_patch2000": {
            "description": "Track4 alternate-backbone retune: TransformerNet with 2000-point patches.",
            "type": '"TransformerNet"',
            "embedding_size": "128",
            "attention_layers_n_heads": "[8, 8, 8, 8]",
            "patch_size": "2000",
            "dropout": "0.0",
        },
    }
    ...
    rows = []
    row_num = 0
    for seed in [1025, 1026, 1027]:
        group = f"track4_seed{seed}_alt_backbones"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} alternate-backbone retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4ab_{row_num:04d}",
                    "phase": "phaseAB_track4_alt_backbones_retune",
                    "phase_label": "Track4 alternate backbones retune",
                    "launch_mode": "concurrent_3x1n",
                    "launch_group": group,
                    "family": "track4_alt_backbones_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Truly new backbone families after the ConvNet family was mostly exhausted.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Alternate Backbones Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test genuinely new backbone families after exhausting most ConvNet-family improvements",
        "",
        "## Variants",
        "- `avgpool_baseline`",
        "- `efficientnet_b0`",
