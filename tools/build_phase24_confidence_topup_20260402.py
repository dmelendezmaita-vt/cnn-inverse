...
CONFIG_ROOT = REPO / f"src/pytorch/configs/reruns_{DATE_TAG}_phase24"
...
def main() -> None:
    ...
    for nodes, ntrain, gbs in weak_points:
        seed = 503
        cfg_rel = (
            f"src/pytorch/configs/reruns_{DATE_TAG}_phase24/track2_true_weak/"
    ...
    for track_key, meta in track_meta.items():
        for lr, ep in lr_epoch_combos:
            ...
            cfg_rel = (
                f"src/pytorch/configs/reruns_{DATE_TAG}_phase24/{track_key}/phase4_lr/"
    ...
    for track_key, meta in track_meta.items():
        for sub_len in phase4_input_lengths:
            seed = 528
            cfg_rel = (
                f"src/pytorch/configs/reruns_{DATE_TAG}_phase24/{track_key}/phase4_input/"
