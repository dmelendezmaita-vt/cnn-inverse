...
DATE_TAG = "20260419_v100_hh_track4_weight_finalists_confirmation"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_weight_finalists_confirmation_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_weight_finalists_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "baseline_inverse_std": {
            "description": "Track4 weight finalists baseline inverse_std.",
            "target_loss_weight_mode": "inverse_std",
        },
        "baseline_uniform": {
            "description": "Track4 weight finalists uniform weighting.",
            "target_loss_weight_mode": "uniform",
        },
        "baseline_manual_5": {
            "description": "Track4 weight finalists manual emphasis on target 5.",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.0, 1.0, 1.0, 1.0, 2.0, 1.0]",
            "target_loss_weights_normalize": "True",
        },
        "baseline_manual_345": {
            "description": "Track4 weight finalists manual emphasis on targets 3,4,5.",
            "target_loss_weight_mode": "manual",
    ...
    row_num = 0
    for seed in [991, 992]:
        group = f"track4_seed{seed}_weight_finalists"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} weight finalists confirmation {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4wf_{row_num:04d}",
                    "phase": "phaseN_track4_weight_finalists_confirmation",
                    "phase_label": "Track4 weight finalists confirmation",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_weight_finalists_confirmation",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Fresh-seed confirmation of the strongest baseline weighting finalists.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Weight Finalists Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm the baseline-branch weighting finalists on fresh seeds",
        "- compare inverse_std, uniform, manual_5, and manual_345 under weighted Huber",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
