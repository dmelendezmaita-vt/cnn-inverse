...
REGISTRY_CSV = TABLES / f"v100_hh_track4_weight_finalists_confirmation_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"v100_hh_track4_weight_finalists_confirmation_run_log_{DATE_TAG}.json"
...
def active_step_nodes(alloc_job_id: str) -> set[str]:
    proc = subprocess.run(
        ["squeue", "-s", "-h", "-j", str(alloc_job_id), "-o", "%i|%N"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def resolve_allocation(args: argparse.Namespace) -> Tuple[str, str, List[str]]:
    ...
    hosts = hostnames(alloc_nodelist)
...
def build_env(
    ...
    env = os.environ.copy()
    # Keep this path intentionally short; Python multiprocessing resource_sharer
    # uses AF_UNIX sockets under TMPDIR and long project-note paths can exceed the
    # platform socket pathname limit.
...
def launch_group(
    ...
    if dry_run:
        return [
            launch_one(row, alloc_job_id, alloc_nodelist, node_slice, dry_run=True)
...
def dedicated_node_options(hosts: Sequence[str], node_count: int) -> List[List[str]]:
    if len(hosts) < node_count:
        raise RuntimeError(f"Need {node_count} hosts but allocation only has {len(hosts)}")
    if node_count == 1:
        return [[h] for h in hosts]
    if node_count == 2:
        opts = []
        for i in range(0, len(hosts) - 1, 2):
            opts.append(list(hosts[i:i + 2]))
        if not opts:
            opts.append(list(hosts[:2]))
        return opts
    return [list(hosts[:node_count])]
...
def main() -> None:
    ...
    for row in dedicated:
        ...
        results.append(launch_one(row, alloc_job_id, alloc_nodelist, node_slice, dry_run=args.dry_run))

    for (mode, group_id), rows in sorted(grouped.items()):
        # preserve matrix order inside the group
        rows = sorted(rows, key=lambda r: r["row_id"])
        if not args.force_rerun:
            if any(existing.get(r["row_id"], {}).get("status") == "COMPLETED" for r in rows):
                continue
        results.extend(launch_group(rows, alloc_job_id, alloc_nodelist, hosts_for_launch, dry_run=args.dry_run))
