...
NOTES_MD = NOTES / f"v100_hh_track4_g64_candidate_confirmation_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "confirm_base_g128_e400": {
            "description": "Track4 confirmation baseline: gbs128 epochs400 lr3e-4.",
        ...
        "confirm_g64_lr1p5e4_w60c60": {
            "description": "Track4 confirmation candidate: gbs64 lr1.5e-4 warmup60 constant60.",
    ...
    rows = []
    row_num = 0
    for seeds in [(975, 976), (977, 978)]:
        group = f"track4_confirm_{seeds[0]}_{seeds[1]}"
        for seed in seeds:
            for strategy_id, variant in variants.items():
                ...
                updates = {
                    "description": f'"{track_key} {strategy_id} confirmation {DATE_LABEL}"',
                ...
                rows.append(
                    {
                        "row_id": f"v100t4cf_{row_num:04d}",
                        "phase": "phaseG_track4_g64_candidate_confirmation",
                        "phase_label": "Track4 g64 candidate confirmation",
                        "launch_mode": "concurrent_4x1n",
                        "launch_group": group,
                        "family": "track4_g64_candidate_confirmation",
                        "track_key": track_key,
                        "policy": "strong",
                        "nodes": "1",
                        "gpus_per_node": "2",
                        ...
                        "notes": "Fresh-seed confirmation of the best current track4 g64 retune candidate.",
                    }
                )
    ...
    notes = [
        f"# V100 HH Track4 g64 Candidate Confirmation ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- confirm whether the best current track4 g64 low-LR candidate is actually better than the baseline on fresh seeds",
        "- compare only baseline g128/e400 vs candidate g64/lr1.5e-4/w60c60",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
