...
REGISTRY_CSV = TABLES / f"a30_hh_track4_knn_robustness_tuning_registry_{DATE_TAG}.csv"
RUN_LOG_JSON = NOTES / f"a30_hh_track4_knn_robustness_tuning_run_log_{DATE_TAG}.json"
STEP_SCRIPT = REPO / "tools" / "run_classical_baseline_step_20260420.sh"
...
def build_srun_cmd(
    ...
    row: Dict[str, str],
    cpus_per_task_override: Optional[int] = None,
) -> List[str]:
    nodelist = ",".join(node_slice)
    cpus_per_task = int(cpus_per_task_override) if cpus_per_task_override is not None else int(row["cpus_per_node"])
    cmd = ["srun"]
    if cluster:
        cmd.extend(["-M", cluster])
    cmd.extend(
...
def launch_group(
    ...
    if mode != "concurrent_4x1n":
        raise RuntimeError(f"Unsupported launch_mode={mode}; classical robustness package expects concurrent_4x1n")
    ...
    for row, node_slice in zip(group_rows, slices):
        ...
        cmd = build_srun_cmd(cluster, alloc_job_id, node_slice, row, cpus_per_task_override=cpus_per_task_override)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Launch Track4 A30 classical robustness rows inside an existing A30 allocation.")
    ...
    ap.add_argument("--cpus-per-task-override", type=int, default=None)
    ...
    if not pending_rows:
        print("All requested classical robustness rows already completed.")
        return
    ...
    for group_name in sorted(pending_by_group):
        ...
        group_results = launch_group(
            ...
            cpus_per_task_override=args.cpus_per_task_override,
        )
