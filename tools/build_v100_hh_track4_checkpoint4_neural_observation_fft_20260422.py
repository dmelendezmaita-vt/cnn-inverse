...
DATE_TAG = "20260422_v100_hh_track4_checkpoint4_neural_observation_fft"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_checkpoint4_neural_observation_fft_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_checkpoint4_neural_observation_fft_notes_{DATE_TAG}.md"
...
def create_config(base_rel: str, out_rel: str, updates: Dict[str, str]) -> str:
    ...
    for k, v in updates.items():
        ...
        if k == "description":
            ...
            raise KeyError(f"Unhandled config key in neural observation FFT builder: {k}")
...
def main() -> None:
    ...
    variants = {
        "effnet_beta005_invvar_e500_fft": {
            "description": "Locked EfficientNet winner retrained with raw+FFT observation encoding.",
            "dropout": "0.1",
        ...
        "effnet_beta003_invvar_e400_fft": {
            "description": "Nearest confirmed EfficientNet neighbor retrained with raw+FFT observation encoding.",
            "dropout": "0.1",
            "learning_rate": "1.5e-4",
            "loss_type": "huber",
            "huber_beta": "0.03",
    ...
    for strategy_id, variant in variants.items():
        group = f"track4_v100_checkpoint4_neural_observation_fft_{strategy_id}"
        for seed in [1071, 1072, 1073, 1074]:
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} checkpoint4 neural observation fft {DATE_LABEL}"',
                "type": '"EfficientNet"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4fft_{row_num:04d}",
                    "phase": "phaseCP4_track4_v100_neural_observation_fft",
                    "phase_label": "Track4 V100 neural observation FFT",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_v100_checkpoint4_neural_observation_fft",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Checkpoint4 neural observation-richness FFT retrain on Falcon V100.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Checkpoint4 Neural Observation FFT ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test the repo-supported neural observation-richness lever via raw trace plus FFT features",
        "- compare the locked EfficientNet winner and nearest confirmed neighbor under a richer clean-data observation encoding",
        "- decide whether Workstream E should keep escalating on the neural side before deeper checkpoint5 analysis",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
