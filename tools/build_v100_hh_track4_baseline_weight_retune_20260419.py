...
DATE_TAG = "20260419_v100_hh_track4_baseline_weight_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_baseline_weight_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_baseline_weight_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "baseline_inverse_std": {
            "description": "Track4 baseline-weight retune baseline inverse_std.",
            "target_loss_weight_mode": "inverse_std",
        },
        "baseline_uniform": {
            "description": "Track4 baseline-weight retune uniform weighting.",
            "target_loss_weight_mode": "uniform",
        },
        "baseline_manual_345": {
            "description": "Track4 baseline-weight retune manual emphasis on targets 3,4,5.",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.0, 1.0, 1.2, 1.2, 1.5, 1.0]",
            "target_loss_weights_normalize": "True",
        },
        "baseline_manual_5": {
            "description": "Track4 baseline-weight retune manual emphasis on target 5 only.",
            "target_loss_weight_mode": "manual",
    ...
    row_num = 0
    for seed in [989, 990]:
        group = f"track4_seed{seed}_baseline_weight"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} baseline-weight retune {DATE_LABEL}"',
            ...
            rows.append(
                {
                    "row_id": f"v100t4bw_{row_num:04d}",
                    "phase": "phaseM_track4_baseline_weight_retune",
                    "phase_label": "Track4 baseline weight retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_baseline_weight_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Track4 baseline-branch weighted Huber retune.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Baseline Weight Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- test whether weighted Huber helps the strongest current track4 baseline branch",
        "- avoid confounding with the weaker g64 branch",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
