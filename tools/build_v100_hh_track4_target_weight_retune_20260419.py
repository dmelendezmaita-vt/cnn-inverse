...
DATE_TAG = "20260419_v100_hh_track4_target_weight_retune"
...
MATRIX_CSV = TABLES / f"v100_hh_track4_target_weight_retune_matrix_{DATE_TAG}.csv"
NOTES_MD = NOTES / f"v100_hh_track4_target_weight_retune_notes_{DATE_TAG}.md"
...
def main() -> None:
    ...
    variants = {
        "baseline_g128_inverse_std": {
            "description": "Track4 weighting retune baseline: g128/e400 inverse_std.",
        ...
        "g64_lowlr_inverse_std": {
            "description": "Track4 weighting retune current candidate: g64 low-lr inverse_std.",
        ...
        "g64_lowlr_manual_345": {
            "description": "Track4 weighting retune: manual emphasis on targets 3,4,5.",
            "learning_rate": "1.5e-4",
            "epochs": "400",
            "init_learning_rate": "7.5e-6",
            "linear_epochs": "60",
            ...
            "global_train_batch_size": "64",
            "train_batch_size": "64",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.0, 1.0, 1.2, 1.2, 1.5, 1.0]",
            "target_loss_weights_normalize": "True",
            "loss_type": "huber",
            "huber_beta": "1.0",
        },
        "g64_lowlr_manual_5": {
            "description": "Track4 weighting retune: manual emphasis on target 5 only.",
            "learning_rate": "1.5e-4",
            "epochs": "400",
            "init_learning_rate": "7.5e-6",
            "linear_epochs": "60",
            ...
            "train_batch_size": "64",
            "target_loss_weight_mode": "manual",
            "target_loss_weights": "[1.0, 1.0, 1.0, 1.0, 2.0, 1.0]",
            "target_loss_weights_normalize": "True",
            "loss_type": "huber",
            "huber_beta": "1.0",
        },
    ...
    row_num = 0
    for seed in [979, 980]:
        group = f"track4_seed{seed}_target_weight"
        for strategy_id, variant in variants.items():
            ...
            updates = {
                "description": f'"{track_key} {strategy_id} target-weight retune {DATE_LABEL}"',
                ...
                "huber_beta": variant.get("huber_beta", "1.0"),
            ...
            if "target_loss_weights" in variant:
                updates["target_loss_weights"] = variant["target_loss_weights"]
                updates["target_loss_weights_normalize"] = variant["target_loss_weights_normalize"]
            create_config(meta["base"], cfg_rel, updates)
            rows.append(
                {
                    "row_id": f"v100t4tw_{row_num:04d}",
                    "phase": "phaseH_track4_target_weight_retune",
                    "phase_label": "Track4 target-weight retune",
                    "launch_mode": "concurrent_4x1n",
                    "launch_group": group,
                    "family": "track4_target_weight_retune",
                    "track_key": track_key,
                    "policy": "strong",
                    "nodes": "1",
                    "gpus_per_node": "2",
                    ...
                    "notes": "Target-weight retune around the best current track4 g64 low-lr branch.",
                }
            )
    ...
    notes = [
        f"# V100 HH Track4 Target-Weight Retune ({DATE_LABEL})",
        "",
        f"- matrix csv: `{MATRIX_CSV}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Purpose",
        "- resolve the track4 metric tradeoff by testing manual target weighting around the best current g64 low-LR branch",
        "- compare baseline, inverse_std candidate, and two manual target-weight settings on fresh seeds",
    ]
    write_text(NOTES_MD, "\n".join(notes) + "\n")
